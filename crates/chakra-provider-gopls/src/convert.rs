...
//! syntax snapshot, and every converted fact carries `Provenance::Gopls`.
...
mod tests {
    ...
    fn go_document(path: &RepoRelativePath, source: &str) -> ProviderDocument {
        ProviderDocument {
            path: path.clone(),
            source: Arc::from(source),
            language: chakra_domain::symbol::Language::Go,
        }
    }

    fn item(name: &str, uri: Uri) -> CallHierarchyItem {
        let selection = Range::new(Position::new(0, 5), Position::new(0, 11));
        CallHierarchyItem {
            name: name.to_owned(),
    ...
    #[test]
    fn incoming_relations_carry_gopls_provenance_and_bounded_call_sites()
    -> Result<(), Box<dyn Error>> {
        ...
        let workspace = ProviderWorkspace::from_documents(
            ...
            Revision(4),
            vec![go_document(&path, "func target() {}\n")],
        );
        ...
        assert_eq!(converted[0].provenance, Provenance::Gopls);
        assert!(!truncated);
        Ok(())
    }
    ...
    fn relations_outside_the_workspace_are_dropped_before_the_limit() -> Result<(), Box<dyn Error>>
    {
        ...
        let workspace = ProviderWorkspace::from_documents(
            repository.path().to_path_buf(),
            Revision(4),
            vec![go_document(&path, "func inside() {}\n")],
        );
