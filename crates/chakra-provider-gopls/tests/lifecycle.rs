//! Hermetic gopls lifecycle regressions using a tiny scripted stdio LSP peer.
...
fn compile_fake_server(root: &Path, name: &str) -> Result<PathBuf, Box<dyn Error>> {
    ...
    let status = Command::new(rustc)
        ...
        .status()?;
    if !status.success() {
        return Err("failed to compile the fake gopls peer".into());
    }
    Ok(executable)
}
...
fn materialize_fake_server(root: &Path, name: &str) -> Result<PathBuf, Box<dyn Error>> {
    ...
    let shared = match SERVER.get_or_init(|| {
        ...
        let executable = compile_fake_server(scratch.path(), "fake-gopls-shared")
...
fn config(executable: &Path) -> GoplsConfig {
    GoplsConfig {
        command: GoplsCommand::stdio(executable.as_os_str().to_owned()),
...
#[test]
fn precise_incoming_call_carries_gopls_provenance() -> Result<(), Box<dyn Error>> {
    let repository = tempfile::tempdir()?;
    let executable = materialize_fake_server(repository.path(), "fake-gopls-precise")?;
    let request = request(repository.path(), Revision(1))?;
    let provider = GoplsProvider::start(request.workspace.clone(), config(&executable))?;
    ...
    assert_eq!(relation.provenance, Provenance::Gopls);
...
fn revision_delta_syncs_only_opened_documents_with_full_text() -> Result<(), Box<dyn Error>> {
    let repository = tempfile::tempdir()?;
    let executable = materialize_fake_server(repository.path(), "fake-gopls-delta")?;
    let mut request = request(repository.path(), Revision(1))?;
    let other_path = RepoRelativePath::new("other.go")?;
    fs::write(
    ...
    request.workspace = workspace(
        ...
        Revision(1),
        vec![
            document(&RepoRelativePath::new("service.go")?, TARGET_SOURCE),
    ...
    let revision_three = PreciseQueryRequest {
        workspace: workspace(
            ...
            vec![
                ProviderDocument {
                    path: RepoRelativePath::new("service.go")?,
...
fn timed_out_request_is_cancelled_and_reports_catching_up() -> Result<(), Box<dyn Error>> {
    let repository = tempfile::tempdir()?;
    let executable = materialize_fake_server(repository.path(), "fake-gopls-hang")?;
    let request = request(repository.path(), Revision(1))?;
    let provider = GoplsProvider::start(request.workspace.clone(), config(&executable))?;
...
fn per_query_wait_budget_returns_before_the_request_timeout() -> Result<(), Box<dyn Error>> {
    let repository = tempfile::tempdir()?;
    let executable = materialize_fake_server(repository.path(), "fake-gopls-hang-budget")?;
    let request = request(repository.path(), Revision(1))?;
    ...
    let provider = GoplsProvider::start(request.workspace.clone(), bounded)?;
...
fn transport_crash_restarts_once_then_degrades() -> Result<(), Box<dyn Error>> {
    let repository = tempfile::tempdir()?;
    let executable = materialize_fake_server(repository.path(), "fake-gopls-crash")?;
    let request = request(repository.path(), Revision(1))?;
    let provider = GoplsProvider::start(request.workspace.clone(), config(&executable))?;
...
fn missing_call_hierarchy_capability_degrades() -> Result<(), Box<dyn Error>> {
    let repository = tempfile::tempdir()?;
    let executable = materialize_fake_server(repository.path(), "fake-gopls-no-hierarchy")?;
    let request = request(repository.path(), Revision(1))?;
    let provider = GoplsProvider::start(request.workspace.clone(), config(&executable))?;
...
fn shutdown_reaps_provider_process_group_descendants() -> Result<(), Box<dyn Error>> {
    let repository = tempfile::tempdir()?;
    let executable = materialize_fake_server(repository.path(), "fake-gopls-spawn-child")?;
    let request = request(repository.path(), Revision(1))?;
    let provider = GoplsProvider::start(request.workspace.clone(), config(&executable))?;
    let result = provider.enrich(request);
    assert_eq!(result.state, ProviderState::Ready);
