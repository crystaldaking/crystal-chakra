...
//!
//! terraform-ls has no rust-analyzer-style `experimental/serverStatus`
//! quiescence signal. Readiness is instead proven by a post-synchronization
//! references request after the current document generation is sent. Incoming
//! callers use references plus document symbols; outgoing callers remain on
//! Chakra's bounded syntax graph because the server has no call hierarchy.
...
use lsp_types::{
    ...
    DocumentSymbolResponse, FileChangeType, FileEvent, InitializeParams, InitializeResult,
    Location, OneOf, PartialResultParams, ReferenceContext, ReferenceParams, SymbolInformation,
...
use crate::convert::{
    CallerSymbol, convert_references, directory_uri, find_symbol_position, path_to_uri, uri_to_path,
};
use crate::{Command, SharedState, TerraformLsConfig};
...
pub(crate) enum ProviderError {
    #[error("terraform-ls transport failed: {0}")]
    Transport(String),
    #[error("timed out waiting for the terraform-ls response")]
    Timeout,
    #[error("the terraform-ls request was cancelled by its caller")]
    Cancelled,
    #[error("terraform-ls request failed ({code}): {message}")]
    Server { code: i32, message: String },
    #[error("invalid terraform-ls response: {0}")]
    InvalidResponse(#[from] serde_json::Error),
    #[error("terraform-ls does not advertise definition, references, and document symbols")]
    Unsupported,
...
impl Worker {
    pub(crate) fn new(
        ...
        let (workspace_documents, workspace_source_bytes) =
            initial_workspace.document_stats_matching(terraform_ls_language);
    ...
    fn query_with_owned_session(
        ...
        let mut session = self
            .session
            .take()
            .ok_or_else(|| ProviderError::Transport("no active terraform-ls process".to_owned()))?;
    ...
    fn query_session(
        ...
        let references = self.references(session, request, deadline)?;
        let symbols = if request.directions.incoming {
            self.caller_symbols(session, &request.workspace, &references, deadline)?
        } else {
            Vec::new()
        };
        // The final references/document-symbol round trip is the
        // post-synchronization barrier. Caller documents discovered through
        // references are opened before their document-symbol request.
        ...
        Ok(PreciseQueryResult {
            ...
            fallback_cause: None,
            incoming,
            // terraform-ls has no callHierarchy. Chakra's bounded
            // Tree-sitter configuration-reference graph remains the outgoing equivalent.
            outgoing: Vec::new(),
            incoming_truncated,
    ...
    fn references(
        ...
        Ok(self
            .send_request::<_, Option<Vec<Location>>>(
                session,
    ...
    fn caller_symbols(
        ...
        references: &[Location],
        deadline: Instant,
    ) -> Result<Vec<CallerSymbol>, ProviderError> {
        ...
        for (path, uri) in documents {
            ...
            if !self.opened_versions.contains_key(&path) {
                ...
                self.sync_metrics.opened_documents = self.open_versions_len();
                self.sync_metrics.text_documents_sent =
                    self.sync_metrics.text_documents_sent.saturating_add(1);
                self.sync_metrics.text_bytes_sent =
                    self.sync_metrics.text_bytes_sent.saturating_add(bytes);
                self.sync_metrics.total_text_documents_sent = self
                    .sync_metrics
                    .total_text_documents_sent
                    .saturating_add(1);
                self.sync_metrics.total_text_bytes_sent = self
                    .sync_metrics
            ...
            let response = self
                .send_request::<_, Option<DocumentSymbolResponse>>(
                    session,
            ...
            match response {
                DocumentSymbolResponse::Flat(symbols) => {
                    callers.extend(
                        symbols
                            .into_iter()
                            .filter_map(|symbol| flat_caller_symbol(symbol, &path, workspace)),
                    );
                }
    ...
    fn start_session(&mut self) -> Result<(), ProviderError> {
        self.set_progress(ProviderProgress {
            ...
            source: ProviderProgressSource::Chakra,
            message: Some("starting the terraform-ls process".to_owned()),
            percentage: None,
        });
    ...
    fn drain_session_events(&mut self) {
        ...
        if closed {
            ...
            self.set_state(
                ProviderState::Degraded,
                None,
                Some("terraform-ls process closed its output".to_owned()),
            );
        } else {
            self.session = Some(session);
        }
...
/// HCL documents synchronized through terraform-ls.
...
mod tests {
    ...
    fn language_ids_follow_the_file_extension() -> Result<(), Box<dyn std::error::Error>> {
        assert_eq!(language_id(&RepoRelativePath::new("main.tf")?), "terraform");
        assert_eq!(
            language_id(&RepoRelativePath::new("tests/main.tftest.hcl")?),
            "terraform"
        );
        assert_eq!(
            language_id(&RepoRelativePath::new("production.tfvars")?),
            "terraform-vars"
        );
        assert_eq!(language_id(&RepoRelativePath::new("job.hcl")?), "hcl");
        Ok(())
    }
}
