//! Deterministic hcl syntax indexing on the shared language-neutral driver
...
use crate::discovery::discover_hcl_sources;
...
pub type HclIndexError = chakra_language_index::LanguageIndexError;
/// Latest source text plus role/package metadata from the same scan.
pub type HclSources = chakra_language_index::LanguageSources;
/// Reusable per-file syntax facts and per-owner relationship contributions.
pub type HclSyntaxIndex = chakra_language_index::LanguageSyntaxIndex<HclHooks>;
/// Complete private initial index, ready for atomic publication.
pub type IndexReport = chakra_language_index::IndexReport<HclHooks>;
/// Reconcile outcome for the syntax index.
pub type ReconcileReport = chakra_language_index::ReconcileReport<HclHooks>;

/// Language seams of the shared indexing driver.
#[derive(Debug, Clone, Copy, Default)]
pub struct HclHooks;

impl LanguageHooks for HclHooks {
    type Parser = HclParser;
    ...
    fn discover_sources(root: &Path) -> Result<Vec<ClassifiedSource>, HclIndexError> {
        Ok(discover_hcl_sources(root)?)
    }
}
...
impl LanguageParser for HclParser {
    fn parse(
        ...
        source: Arc<str>,
    ) -> Result<chakra_language_index::ParsedFile, HclIndexError> {
        HclParser::parse(self, path, source)
            .map_err(|error| HclIndexError::Parse(error.to_string()))
    }
}
...
pub fn index_repository(root: &Path) -> Result<IndexReport, HclIndexError> {
    chakra_language_index::index_repository::<HclHooks>(root)
}
...
pub fn scan_repository_sources(repository_root: &Path) -> Result<HclSources, HclIndexError> {
    chakra_language_index::scan_repository_sources::<HclHooks>(repository_root)
}
...
mod tests {
    ...
    fn cancelling_parallel_parse_joins_every_scoped_worker() -> Result<(), Box<dyn Error>> {
        ...
        let indexing = std::thread::spawn(move || {
            ...
            parse_sources_scheduled_observed::<HclHooks>(
                sources,
                WORKERS,
                1,
                &worker_cancellation,
