//! Deterministic Hcl syntax indexing with reusable per-file facts.
...
use crate::discovery::{DiscoveryError, discover_hcl_sources, resolve_repository_root};
use crate::parser::{HclParser, ParsedFile, SymbolDraft};
...
pub struct ReconcileReport {
    ...
    pub next_index: Option<HclSyntaxIndex>,
...
pub struct IndexReport {
    ...
    pub syntax_index: HclSyntaxIndex,
}

/// Latest Hcl source text plus role/package metadata from the same scan.
#[derive(Debug, Clone, Default, PartialEq, Eq)]
pub struct HclSources {
...
/// Failure to discover, read, parse, or validate the Hcl syntax index.
#[derive(Debug, Error)]
pub enum HclIndexError {
    #[error(transparent)]
    Discovery(#[from] DiscoveryError),
    #[error("failed to read Hcl source {path}: {source}")]
    Read {
        path: RepoRelativePath,
        #[source]
        source: io::Error,
    },
    #[error("Hcl source {path} exceeds the {limit}-byte indexing budget")]
    SourceTooLarge {
        path: RepoRelativePath,
        limit: usize,
    },
    #[error("indexed Hcl sources exceed the {limit}-byte repository budget")]
    RepositoryTooLarge { limit: usize },
    #[error("failed to parse Hcl source: {0}")]
    Parse(String),
    #[error("failed to start a bounded Hcl parser worker: {0}")]
    WorkerSpawn(#[source] io::Error),
    #[error("a bounded Hcl parser worker panicked")]
    WorkerPanicked,
    #[error("Hcl syntax index update failed: {0}")]
    Update(String),
    #[error(transparent)]
    Graph(#[from] GraphError),
    #[error("constructed syntax graph is inconsistent: {0}")]
    Consistency(#[from] ConsistencyError),
    #[error("Hcl syntax indexing was cancelled")]
    Cancelled,
}
...
impl Default for HclSyntaxIndex {
    fn default() -> Self {
        Self {
            files: BTreeMap::new(),
...
impl HclSyntaxIndex {
    ...
    ) -> Result<(Self, SymbolGraph), HclIndexError> {
    ...
    ) -> Result<(Self, SymbolGraph, LanguageBuildMetrics), HclIndexError> {
    ...
    pub fn reconcile_sources(
        &self,
        sources: BTreeMap<RepoRelativePath, Arc<str>>,
    ) -> Result<ReconcileReport, HclIndexError> {
    ...
    pub fn reconcile_classified_sources_bounded(
        ...
        let mut parser = HclParser::new().map_err(parse_error)?;
    ...
    fn materialize_graph_bounded(
        &self,
        cancellation: &IndexCancellation,
    ) -> Result<(SymbolGraph, GraphBuildReport), HclIndexError> {
    ...
    fn materialize_graph_delta(
        ...
        cancellation: &IndexCancellation,
    ) -> Result<Option<(SymbolGraph, GraphBuildReport)>, HclIndexError> {
        ...
        for path in changed_paths {
            ...
            if stable_symbol_paths.contains(path) {
                let file = self
                    .files
                    .get(path)
                    .ok_or_else(|| HclIndexError::Update(format!("missing changed file {path}")))?;
                let ids: Vec<_> = graph
...
fn parse_sources_scheduled(
    ...
    parallel_file_threshold: usize,
    cancellation: &IndexCancellation,
) -> Result<(BTreeMap<RepoRelativePath, Arc<ParsedFile>>, ParseSchedule), HclIndexError> {
...
fn parse_sources_scheduled_inner(
    ...
    let outputs = thread::scope(|scope| -> Result<Vec<ParseWorkerOutput>, HclIndexError> {
        let mut workers = Vec::with_capacity(worker_count);
        let mut spawn_error = None;
        for worker in 0..worker_count {
            let name = format!("chakra-hcl-parser-{worker}");
            match thread::Builder::new().name(name).spawn_scoped(scope, || {
                ...
                let output = (|| {
                    let mut parser = match HclParser::new() {
                        Ok(parser) => parser,
                        Err(error) => {
        ...
        if worker_panicked {
            return Err(HclIndexError::WorkerPanicked);
        }
        if let Some(error) = spawn_error {
            return Err(HclIndexError::WorkerSpawn(error));
        }
        Ok(outputs)
    })?;
    check_cancelled(cancellation)?;
    ...
    {
        return Err(HclIndexError::Parse(error));
    }
    ...
    for output in outputs {
        for (index, parsed) in output.parsed {
            let Some(slot) = ordered.get_mut(index) else {
                return Err(HclIndexError::Update(
    ...
    for ((path, _), parsed) in inputs.into_iter().zip(ordered) {
        let parsed = parsed.ok_or_else(|| {
            HclIndexError::Update("parser worker omitted an admitted source".to_owned())
        })?;
        let parsed = parsed.map_err(HclIndexError::Parse)?;
        files.insert(path, Arc::new(parsed));
    }
    Ok((
        files,
...
fn check_cancelled(cancellation: &IndexCancellation) -> Result<(), HclIndexError> {
    if cancellation.is_cancelled() {
        Err(HclIndexError::Cancelled)
    } else {
        Ok(())
    }
}
...
fn build_all_relationships(
    ...
    limit: u64,
    cancellation: &IndexCancellation,
) -> Result<BTreeMap<RepoRelativePath, Arc<RelationshipContribution>>, HclIndexError> {
...
fn parse_error(error: impl std::fmt::Display) -> HclIndexError {
    HclIndexError::Parse(error.to_string())
}
...
fn read_sources(repository_root: &Path) -> Result<HclSources, HclIndexError> {
    let files = discover_hcl_sources(repository_root)?;
    ...
    for discovered in files {
        ...
        file.take((MAX_SOURCE_FILE_BYTES + 1) as u64)
            ...
            .map_err(|source| HclIndexError::Read {
                path: path.clone(),
                source,
            })?;
        if source.len() > MAX_SOURCE_FILE_BYTES {
            return Err(HclIndexError::SourceTooLarge {
                path,
        ...
        total_bytes =
            total_bytes
                ...
                .ok_or(HclIndexError::RepositoryTooLarge {
                    limit: MAX_REPOSITORY_SOURCE_BYTES,
                })?;
        if total_bytes > MAX_REPOSITORY_SOURCE_BYTES {
            return Err(HclIndexError::RepositoryTooLarge {
                limit: MAX_REPOSITORY_SOURCE_BYTES,
            });
        }
...
/// Builds a complete Hcl syntax index from the actual materialized Git
/// worktree. The caller owns atomic publication into `WorkspaceEngine`.
pub fn index_repository(root: &Path) -> Result<IndexReport, HclIndexError> {
    let started = Instant::now();
    let repository_root = resolve_repository_root(root)?;
    let span = info_span!("hcl_repository_index", root = %repository_root.display());
    let _entered = span.enter();
    let sources = read_sources(&repository_root)?;
    let discovered_files = sources.files.len() as u64;
    let (syntax_index, graph) = HclSyntaxIndex::from_classified_sources(sources)?;
...
/// Reads the latest Git-aware Hcl file inventory and exact contents.
pub fn scan_repository_sources(repository_root: &Path) -> Result<HclSources, HclIndexError> {
...
mod tests {
    ...
    fn bounded_parallel_parsing_is_deterministic() -> Result<(), Box<dyn Error>> {
        ...
        for index in 0..64 {
            sources.insert(
                RepoRelativePath::new(format!("src/generated_{index:03}.tf"))?,
                Arc::<str>::from(format!(
                    "resource \"null_resource\" \"caller_{index}\" {{ triggers = {{ value = null_resource.helper_{index}.id }} }}\nresource \"null_resource\" \"helper_{index}\" {{}}\n"
                )),
            );
        }
        let cancellation = IndexCancellation::default();
        let (_, sequential, sequential_metrics) = HclSyntaxIndex::from_sources_scheduled(
            sources.clone(),
            GraphBuildLimits::UNLIMITED,
            1,
            1,
        ...
        let (_, parallel, parallel_metrics) = HclSyntaxIndex::from_sources_scheduled(
            sources,
            GraphBuildLimits::UNLIMITED,
            4,
            1,
    ...
    fn cancelling_parallel_parse_joins_every_scoped_worker() -> Result<(), Box<dyn Error>> {
        ...
        for index in 0..64 {
            sources.insert(
                RepoRelativePath::new(format!("src/cancel_{index:03}.tf"))?,
                Arc::<str>::from(format!(
                    "resource \"null_resource\" \"item_{index}\" {{}}\n"
                )),
            );
        }
        let cancellation = IndexCancellation::default();
        ...
        assert!(matches!(result, Err(HclIndexError::Cancelled)));
        Ok(())
    }
    ...
    fn duplicate_call_fanout_is_stored_linearly() -> Result<(), Box<dyn Error>> {
        ...
        for index in 0..TARGETS {
            sources.insert(
                RepoRelativePath::new(format!("src/target_{index:03}.tf"))?,
                Arc::<str>::from("resource \"null_resource\" \"target\" {}\n"),
            );
        }
        for index in 0..CALLS {
            sources.insert(
                RepoRelativePath::new(format!("src/caller_{index:03}.tf"))?,
                Arc::<str>::from(format!(
                    "resource \"null_resource\" \"caller_{index}\" {{ triggers = {{ target = null_resource.target.id }} }}\n"
                )),
            );
        }

        let started = Instant::now();
        let (_, graph) = HclSyntaxIndex::from_sources(sources)?;
    ...
    fn truncated_catalog_never_turns_ambiguity_into_a_unique_call() -> Result<(), Box<dyn Error>> {
        let mut sources = BTreeMap::new();
        sources.insert(
            RepoRelativePath::new("src/a_caller.tf")?,
            Arc::<str>::from(
                "resource \"null_resource\" \"invoke\" { triggers = { target = null_resource.target.id } }\n",
            ),
        );
        sources.insert(
            RepoRelativePath::new("src/b_target.tf")?,
            Arc::<str>::from("resource \"null_resource\" \"target\" {}\n"),
        );
        sources.insert(
            RepoRelativePath::new("src/c_target.tf")?,
            Arc::<str>::from("resource \"null_resource\" \"target\" {}\n"),
        );
        let cancellation = IndexCancellation::default();
        let (_, graph, metrics) = HclSyntaxIndex::from_sources_bounded(
            sources,
            GraphBuildLimits {
                max_symbols: 2,
                max_edges: 10,
        ...
        // The caller contributes a physical module, resource, and property;
        // each target contributes a module and resource. With a two-symbol
        // budget the remaining five symbols are omitted.
        assert_eq!(metrics.graph.omitted_symbols, 5);
    ...
    fn metadata_change_republishes_without_reparsing_source() -> Result<(), Box<dyn Error>> {
        let path = RepoRelativePath::new("src/main.tf")?;
        let source = Arc::<str>::from("resource \"null_resource\" \"stable\" {}\n");
        let initial = HclSources {
            files: BTreeMap::from([(path.clone(), source.clone())]),
            metadata: BTreeMap::from([(path.clone(), SourceMetadata::path_fallback(&path))]),
        };
        let (index, _) = HclSyntaxIndex::from_classified_sources(initial)?;
        let changed = HclSources {
            ...
            metadata: BTreeMap::from([(
                ...
                SourceMetadata {
                    ...
                    classification:
                        chakra_domain::source::SourceClassification::TerraformModuleMetadata,
    ...
    fn rejects_a_source_larger_than_the_file_budget() -> Result<(), Box<dyn Error>> {
        ...
        let file = File::create(repository.path().join("src/large.tf"))?;
        ...
        assert!(matches!(
            error,
            HclIndexError::SourceTooLarge { limit, .. } if limit == MAX_SOURCE_FILE_BYTES
        ));
        Ok(())
    }
}
