//! Deterministic Java syntax indexing with reusable per-file facts.
...
use crate::discovery::{DiscoveryError, discover_java_sources, resolve_repository_root};
use crate::parser::{JavaParser, ParsedFile, SymbolDraft};
...
pub struct ReconcileReport {
    ...
    pub next_index: Option<JavaSyntaxIndex>,
...
pub struct IndexReport {
    ...
    pub syntax_index: JavaSyntaxIndex,
}

/// Latest Java source text plus role/package metadata from the same scan.
...
/// Failure to discover, read, parse, or validate the Java syntax index.
#[derive(Debug, Error)]
pub enum JavaIndexError {
    #[error(transparent)]
    Discovery(#[from] DiscoveryError),
    #[error("failed to read Java source {path}: {source}")]
    Read {
        path: RepoRelativePath,
        #[source]
        source: io::Error,
    },
    #[error("Java source {path} exceeds the {limit}-byte indexing budget")]
    SourceTooLarge {
        path: RepoRelativePath,
        limit: usize,
    },
    #[error("indexed Java sources exceed the {limit}-byte repository budget")]
    RepositoryTooLarge { limit: usize },
    #[error("failed to parse Java source: {0}")]
    Parse(String),
    #[error("failed to start a bounded Java parser worker: {0}")]
    WorkerSpawn(#[source] io::Error),
    #[error("a bounded Java parser worker panicked")]
    WorkerPanicked,
    #[error("Java syntax index update failed: {0}")]
    Update(String),
    #[error(transparent)]
    Graph(#[from] GraphError),
    #[error("constructed syntax graph is inconsistent: {0}")]
    Consistency(#[from] ConsistencyError),
    #[error("Java syntax indexing was cancelled")]
    Cancelled,
}
...
impl Default for JavaSyntaxIndex {
    fn default() -> Self {
        Self {
            files: BTreeMap::new(),
...
impl JavaSyntaxIndex {
    pub fn from_sources(
        sources: BTreeMap<RepoRelativePath, Arc<str>>,
    ) -> Result<(Self, SymbolGraph), JavaIndexError> {
    ...
    ) -> Result<(Self, SymbolGraph, LanguageBuildMetrics), JavaIndexError> {
    ...
    pub fn reconcile_sources(
        &self,
        sources: BTreeMap<RepoRelativePath, Arc<str>>,
    ) -> Result<ReconcileReport, JavaIndexError> {
    ...
    pub fn reconcile_classified_sources_bounded(
        ...
        let mut parser = JavaParser::new().map_err(parse_error)?;
    ...
    fn materialize_graph_bounded(
        &self,
        cancellation: &IndexCancellation,
    ) -> Result<(SymbolGraph, GraphBuildReport), JavaIndexError> {
    ...
    fn materialize_graph_delta(
        ...
        cancellation: &IndexCancellation,
    ) -> Result<Option<(SymbolGraph, GraphBuildReport)>, JavaIndexError> {
        ...
        for path in changed_paths {
            ...
            if stable_symbol_paths.contains(path) {
                let file = self.files.get(path).ok_or_else(|| {
                    JavaIndexError::Update(format!("missing changed file {path}"))
                })?;
                let ids: Vec<_> = graph
...
fn parse_sources_scheduled(
    ...
    parallel_file_threshold: usize,
    cancellation: &IndexCancellation,
) -> Result<(BTreeMap<RepoRelativePath, Arc<ParsedFile>>, ParseSchedule), JavaIndexError> {
...
fn parse_sources_scheduled_inner(
    ...
    let outputs = thread::scope(|scope| -> Result<Vec<ParseWorkerOutput>, JavaIndexError> {
        let mut workers = Vec::with_capacity(worker_count);
        let mut spawn_error = None;
        for worker in 0..worker_count {
            let name = format!("chakra-java-parser-{worker}");
        ...
        if worker_panicked {
            return Err(JavaIndexError::WorkerPanicked);
        }
        if let Some(error) = spawn_error {
            return Err(JavaIndexError::WorkerSpawn(error));
        }
        Ok(outputs)
    })?;
    check_cancelled(cancellation)?;
    ...
    {
        return Err(JavaIndexError::Parse(error));
    }
    ...
    for output in outputs {
        for (index, parsed) in output.parsed {
            let Some(slot) = ordered.get_mut(index) else {
                return Err(JavaIndexError::Update(
    ...
    for ((path, _), parsed) in inputs.into_iter().zip(ordered) {
        let parsed = parsed.ok_or_else(|| {
            JavaIndexError::Update("parser worker omitted an admitted source".to_owned())
        })?;
        let parsed = parsed.map_err(JavaIndexError::Parse)?;
        files.insert(path, Arc::new(parsed));
    }
    Ok((
        files,
...
fn check_cancelled(cancellation: &IndexCancellation) -> Result<(), JavaIndexError> {
    if cancellation.is_cancelled() {
        Err(JavaIndexError::Cancelled)
    } else {
        Ok(())
    }
}
...
fn build_all_relationships(
    ...
    limit: u64,
    cancellation: &IndexCancellation,
) -> Result<BTreeMap<RepoRelativePath, Arc<RelationshipContribution>>, JavaIndexError> {
...
fn parse_error(error: impl std::fmt::Display) -> JavaIndexError {
    JavaIndexError::Parse(error.to_string())
}
...
fn read_sources(repository_root: &Path) -> Result<JavaSources, JavaIndexError> {
    let files = discover_java_sources(repository_root)?;
    ...
    for discovered in files {
        ...
        file.take((MAX_SOURCE_FILE_BYTES + 1) as u64)
            ...
            .map_err(|source| JavaIndexError::Read {
                path: path.clone(),
                source,
            })?;
        if source.len() > MAX_SOURCE_FILE_BYTES {
            return Err(JavaIndexError::SourceTooLarge {
                path,
        ...
        total_bytes =
            total_bytes
                ...
                .ok_or(JavaIndexError::RepositoryTooLarge {
                    limit: MAX_REPOSITORY_SOURCE_BYTES,
                })?;
        if total_bytes > MAX_REPOSITORY_SOURCE_BYTES {
            return Err(JavaIndexError::RepositoryTooLarge {
                limit: MAX_REPOSITORY_SOURCE_BYTES,
            });
        }
...
/// Builds a complete Java syntax index from the actual materialized Git
/// worktree. The caller owns atomic publication into `WorkspaceEngine`.
pub fn index_repository(root: &Path) -> Result<IndexReport, JavaIndexError> {
    let started = Instant::now();
    let repository_root = resolve_repository_root(root)?;
    let span = info_span!("java_repository_index", root = %repository_root.display());
    let _entered = span.enter();
    let sources = read_sources(&repository_root)?;
    let discovered_files = sources.files.len() as u64;
    let (syntax_index, graph) = JavaSyntaxIndex::from_classified_sources(sources)?;
...
/// Reads the latest Git-aware Java file inventory and exact contents.
pub fn scan_repository_sources(repository_root: &Path) -> Result<JavaSources, JavaIndexError> {
...
mod tests {
    ...
    fn bounded_parallel_parsing_is_deterministic() -> Result<(), Box<dyn Error>> {
        ...
        for index in 0..64 {
            sources.insert(
                RepoRelativePath::new(format!("src/generated_{index:03}.java"))?,
                Arc::<str>::from(format!(
        ...
        let cancellation = IndexCancellation::default();
        let (_, sequential, sequential_metrics) = JavaSyntaxIndex::from_sources_scheduled(
            sources.clone(),
            GraphBuildLimits::UNLIMITED,
            1,
            1,
        ...
        let (_, parallel, parallel_metrics) = JavaSyntaxIndex::from_sources_scheduled(
            sources,
            GraphBuildLimits::UNLIMITED,
            4,
            1,
    ...
    fn cancelling_parallel_parse_joins_every_scoped_worker() -> Result<(), Box<dyn Error>> {
        ...
        for index in 0..64 {
            sources.insert(
                RepoRelativePath::new(format!("src/cancel_{index:03}.java"))?,
                Arc::<str>::from(format!(
        ...
        assert!(matches!(result, Err(JavaIndexError::Cancelled)));
        Ok(())
    }
    ...
    fn duplicate_call_fanout_is_stored_linearly() -> Result<(), Box<dyn Error>> {
        ...
        let mut sources = BTreeMap::new();
        sources.insert(
            RepoRelativePath::new("src/lib.java")?,
            Arc::<str>::from(source),
        );

        let started = Instant::now();
        let (_, graph) = JavaSyntaxIndex::from_sources(sources)?;
    ...
    fn truncated_catalog_never_turns_ambiguity_into_a_unique_call() -> Result<(), Box<dyn Error>> {
        let mut sources = BTreeMap::new();
        sources.insert(
            RepoRelativePath::new("src/a_caller.java")?,
            Arc::<str>::from("class Invoke { void invoke() { target(); } }\n"),
        );
        sources.insert(
            RepoRelativePath::new("src/b_target.java")?,
            Arc::<str>::from("class Target { void target() {} }\n"),
        );
        sources.insert(
            RepoRelativePath::new("src/c_target.java")?,
            Arc::<str>::from("class Target { void target() {} }\n"),
        );
        let cancellation = IndexCancellation::default();
        let (_, graph, metrics) = JavaSyntaxIndex::from_sources_bounded(
            sources,
            GraphBuildLimits {
                max_symbols: 2,
                max_edges: 10,
        ...
        // Each Java file declares two symbols (the class plus its method):
    ...
    fn rejects_a_source_larger_than_the_file_budget() -> Result<(), Box<dyn Error>> {
        ...
        let file = File::create(repository.path().join("src/large.java"))?;
        ...
        assert!(matches!(
            error,
            JavaIndexError::SourceTooLarge { limit, .. } if limit == MAX_SOURCE_FILE_BYTES
        ));
        Ok(())
    }
}
