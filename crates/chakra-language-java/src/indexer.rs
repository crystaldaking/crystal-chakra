//! Deterministic java syntax indexing on the shared language-neutral driver
...
use crate::discovery::discover_java_sources;
...
pub type JavaIndexError = chakra_language_index::LanguageIndexError;
/// Latest source text plus role/package metadata from the same scan.
pub type JavaSources = chakra_language_index::LanguageSources;
/// Reusable per-file syntax facts and per-owner relationship contributions.
pub type JavaSyntaxIndex = chakra_language_index::LanguageSyntaxIndex<JavaHooks>;
/// Complete private initial index, ready for atomic publication.
pub type IndexReport = chakra_language_index::IndexReport<JavaHooks>;
/// Reconcile outcome for the syntax index.
pub type ReconcileReport = chakra_language_index::ReconcileReport<JavaHooks>;
...
impl LanguageHooks for JavaHooks {
    ...
    fn discover_sources(root: &Path) -> Result<Vec<ClassifiedSource>, JavaIndexError> {
        Ok(discover_java_sources(root)?)
    }
}
...
impl LanguageParser for JavaParser {
    fn parse(
        ...
        source: Arc<str>,
    ) -> Result<chakra_language_index::ParsedFile, JavaIndexError> {
        JavaParser::parse(self, path, source)
            .map_err(|error| JavaIndexError::Parse(error.to_string()))
    }
}
...
pub fn index_repository(root: &Path) -> Result<IndexReport, JavaIndexError> {
    chakra_language_index::index_repository::<JavaHooks>(root)
}
...
pub fn scan_repository_sources(repository_root: &Path) -> Result<JavaSources, JavaIndexError> {
    chakra_language_index::scan_repository_sources::<JavaHooks>(repository_root)
}
...
mod tests {
    ...
    fn cancelling_parallel_parse_joins_every_scoped_worker() -> Result<(), Box<dyn Error>> {
        ...
        let indexing = std::thread::spawn(move || {
            ...
            parse_sources_scheduled_observed::<JavaHooks>(
                sources,
                WORKERS,
                1,
                &worker_cancellation,
