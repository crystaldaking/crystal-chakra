//! Hermetic vtsls lifecycle regressions using a tiny scripted stdio LSP peer.
...
fn main() -> io::Result<()> {
    ...
    loop {
        ...
        if body.contains("\"method\":\"initialize\"") {
            ...
            let item = format!(
                "[{{\"name\":\"target\",\"kind\":12,\"uri\":\"{last_uri}\",\"range\":{{\"start\":{{\"line\":0,\"character\":16}},\"end\":{{\"line\":0,\"character\":22}}}},\"selectionRange\":{{\"start\":{{\"line\":0,\"character\":16}},\"end\":{{\"line\":0,\"character\":22}}}}}}]"
            );
            if let Some(id) = request_id(&body) {
                send(id, &item)?;
            }
            ...
            let call = format!(
                "[{{\"from\":{{\"name\":\"caller\",\"kind\":12,\"uri\":\"{last_uri}\",\"range\":{{\"start\":{{\"line\":1,\"character\":16}},\"end\":{{\"line\":1,\"character\":22}}}},\"selectionRange\":{{\"start\":{{\"line\":1,\"character\":16}},\"end\":{{\"line\":1,\"character\":22}}}}}},\"fromRanges\":[{{\"start\":{{\"line\":2,\"character\":2}},\"end\":{{\"line\":2,\"character\":8}}}}]}}]"
            );
            if let Some(id) = request_id(&body) {
                send(id, &call)?;
            }
...
fn compile_fake_server(root: &Path, name: &str) -> Result<PathBuf, Box<dyn Error>> {
    ...
    let status = Command::new(rustc)
        ...
        .status()?;
    if !status.success() {
        return Err("failed to compile the fake vtsls peer".into());
    }
    Ok(executable)
}
...
fn config(executable: &Path) -> VtslsConfig {
    VtslsConfig {
        command: VtslsCommand::stdio(executable.as_os_str().to_owned()),
...
fn precise_incoming_call_carries_vtsls_provenance() -> Result<(), Box<dyn Error>> {
    ...
    assert_eq!(relation.provenance, Provenance::Vtsls);
...
fn revision_delta_syncs_only_opened_documents_with_full_text() -> Result<(), Box<dyn Error>> {
    ...
    request.workspace = workspace(
        ...
        Revision(1),
        vec![
            document(&RepoRelativePath::new("src/index.ts")?, TARGET_SOURCE),
            document(&other_path, "export const other = 1;\n"),
        ],
    )?;
    let provider = VtslsProvider::start(request.workspace.clone(), config(&executable))?;
    ...
    let revision_two = PreciseQueryRequest {
        workspace: workspace(
            ...
            vec![
                document(&RepoRelativePath::new("src/index.ts")?, TARGET_SOURCE),
                document(&other_path, "export const other = 2;\n"),
            ],
        )?,
        ..request.clone()
    };
...
fn per_query_wait_budget_returns_before_the_request_timeout() -> Result<(), Box<dyn Error>> {
    ...
    let provider = VtslsProvider::start(request.workspace.clone(), bounded)?;
