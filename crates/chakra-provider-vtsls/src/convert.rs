...
mod tests {
    ...
    fn typescript_document(path: &RepoRelativePath, source: &str) -> ProviderDocument {
        ProviderDocument {
            ...
            language: chakra_domain::symbol::Language::TypeScript,
        }
    }

    fn item(name: &str, uri: Uri) -> CallHierarchyItem {
        let selection = Range::new(Position::new(0, 17), Position::new(0, 23));
        CallHierarchyItem {
            name: name.to_owned(),
    ...
    -> Result<(), Box<dyn Error>> {
        ...
        assert_eq!(converted[0].provenance, Provenance::Vtsls);
        assert!(!truncated);
        Ok(())
    }
