...
//! plus source, so exact structural equality implies extracted-fact equality.
//!
...
impl TextEdit {
    ...
    pub fn apply(&self, source: &str) -> Check<String> {
        self.validate(source)?;
        let removed = self.old_end_byte - self.start_byte;
    ...
    pub fn input_edit(&self, old: &str, new: &str) -> Check<InputEdit> {
        self.validate(old)?;
        let new_end_byte = self.start_byte + self.new_text.len();
        let expected_len = old.len() - (self.old_end_byte - self.start_byte) + self.new_text.len();
        ensure(
            new.len() == expected_len
                && new_end_byte <= new.len()
                && new.is_char_boundary(new_end_byte)
                && new[..self.start_byte] == old[..self.start_byte]
                && new[self.start_byte..new_end_byte] == self.new_text
                && new[new_end_byte..] == old[self.old_end_byte..],
            "edited source does not match the text edit",
        )?;
        Ok(InputEdit {
    ...
    fn validate(&self, source: &str) -> Check<()> {
        ensure(
            self.start_byte <= self.old_end_byte && self.old_end_byte <= source.len(),
            "text edit byte range is outside the source",
        )?;
        ensure(
            source.is_char_boundary(self.start_byte) && source.is_char_boundary(self.old_end_byte),
            "text edit byte range splits a UTF-8 code point",
        )?;
        Ok(())
    }
}
...
/// Exact structural comparison of two trees over every node in cursor order.
/// The source text is identical for both trees, so equal grammar node kinds,
/// ranges, flags, and child layout imply equal field lookups and node text.
pub fn trees_are_structurally_equal(left: &Tree, right: &Tree) -> bool {
    let mut left_cursor = left.walk();
    let mut right_cursor = right.walk();
    loop {
        if !nodes_are_structurally_equal(left_cursor.node(), right_cursor.node()) {
            return false;
        }
        let left_child = left_cursor.goto_first_child();
        let right_child = right_cursor.goto_first_child();
        ...
        loop {
            let left_sibling = left_cursor.goto_next_sibling();
            let right_sibling = right_cursor.goto_next_sibling();
            ...
            let left_parent = left_cursor.goto_parent();
            let right_parent = right_cursor.goto_parent();
...
fn nodes_are_structurally_equal(left: Node<'_>, right: Node<'_>) -> bool {
...
pub struct ScenarioOutcome {
    ...
    pub structural_comparisons: u64,
...
fn iteration_edit(
    ...
    match pattern {
        EditPattern::SmallEdit => {
            ...
            ensure(!ranges.is_empty(), "document has no identifier edit sites")?;
...
pub fn run_edit_scenario(
    ...
    for index in 0..iterations {
        ...
        drop(tree.ok_or_else(|| failure(format!("{}: full parse aborted", language.name)))?);
    ...
    // Validate exact structure in a separate untimed pass so correctness
    // checks cannot contaminate either arm's timing samples.
    let mut validation_full_parser = language.parser()?;
    let mut validation_incremental_parser = language.parser()?;
    let mut current = base.to_owned();
    let mut retained = base_tree;
    let mut pending = None;
    let mut mismatches = 0_u64;
    for index in 0..iterations {
        let edit = iteration_edit(pattern, language, base, &current, index, &mut pending)?;
        let next = edit.apply(&current)?;
        retained.edit(&edit.input_edit(&current, &next)?);
        let incremental = validation_incremental_parser
            .parse(&next, Some(&retained))
            .ok_or_else(|| failure(format!("{}: validation parse aborted", language.name)))?;
        let full = validation_full_parser
            .parse(&next, None)
            .ok_or_else(|| failure(format!("{}: validation full parse aborted", language.name)))?;
        if !trees_are_structurally_equal(&incremental, &full) {
    ...
    Ok(ScenarioOutcome {
        ...
        structural_comparisons: iterations as u64,
...
pub fn memory_file_set(language: &BenchLanguage) -> Check<Vec<String>> {
    ...
    files.sort();
    let stride = files.len().div_ceil(MEMORY_SET_MAX_FILES).max(1);
    let mut sources = Vec::new();
    let mut total = 0_u64;
    for (_, size, path) in files.iter().step_by(stride).take(MEMORY_SET_MAX_FILES) {
...
fn cancelled_parse(
    ...
    let canceller = thread::Builder::new()
        .name("chakra-incremental-eval-canceller".to_owned())
        .spawn(move || {
            thread::sleep(delay);
        ...
        .map_err(|error| failure(format!("failed to spawn canceller thread: {error}")))?;
    ...
    let _result = parser.parse_with_options(
...
pub fn render_results_markdown(rows: &[ScenarioRow]) -> String {
    ...
    for row in rows {
        ...
        out.push_str(&format!(
            ...
            row.outcome.structural_mismatches,
            row.outcome.structural_comparisons,
        ));
    }
    out
}
