        ...
        TargetKind::Corpus => {
            ...
            if !dirty.is_empty() {
                return Ok(PersistenceReport::skipped(
                    ...
                    &target.sha,
                    format!(
                        "checkout has local changes; refusing to benchmark or modify it: {dirty:?}"
                    ),
                ));
            }
...
mod tests {
    ...
    #[test]
    fn dirty_corpus_checkout_is_refused_without_discarding_changes() -> Check<()> {
        let (_cache, target) = tiny_checkout()?;
        let checkout = target.checkout.as_ref().ok_or("checkout missing")?;
        let changed = "pub fn local_change_must_survive() {}\n";
        fs::write(checkout.join("src/lib.rs"), changed)?;

        let spool = TempDir::new()?;
        let report = evaluate_target(&target, 1, spool.path())?;

        assert_eq!(report.status, TargetStatus::Skipped);
        assert!(report.skip_reason.contains("local changes"));
        assert_eq!(fs::read_to_string(checkout.join("src/lib.rs"))?, changed);
        Ok(())
    }
