...
use chakra_conformance::incremental::{
    BenchLanguage, CorpusSelection, EditPattern, ScenarioOutcome, ScenarioRow, bench_languages,
    cancellation_latency, cold_parse_timing, corpus_documents, fuzz_edit_equivalence,
    hermetic_source, memory_file_set, render_results_markdown, retained_tree_memory,
    run_edit_scenario,
};
    ...
    for language in bench_languages() {
        ...
        for pattern in &patterns {
            let outcome = run_edit_scenario(&language, &base, pattern, 10)?;
                ...
                outcome.fingerprint_mismatches == 0,
                format!(
                    "{}: {} scenario edits diverged from a full reparse",
                    language.name, outcome.fingerprint_mismatches
                ),
            )?;
        }
    }
