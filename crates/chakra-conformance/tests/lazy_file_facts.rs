...
use chakra_engine::{
    ...
    LazyFactStore, WorkspaceEngine, WorkspaceSnapshot,
};
...
fn input_for<'a>(
    ...
    Ok(FileFactInput::from_snapshot(snapshot, path)?)
}
...
fn compare(snapshot: &WorkspaceSnapshot) -> Result<Comparison, Box<dyn Error>> {
    ...
    for path in &files {
        let fact = producer.compute(&input_for(snapshot, path)?, &operation)?;
    ...
    for path in &workload {
        let first = store.get_or_compute(&input_for(snapshot, path)?, &operation)?;
        let second = store.get_or_compute(&input_for(snapshot, path)?, &operation)?;
