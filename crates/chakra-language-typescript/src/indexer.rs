//! Deterministic TypeScript syntax indexing with reusable per-file facts.
...
use crate::discovery::{DiscoveryError, discover_typescript_sources, resolve_repository_root};
use crate::parser::{ParsedFile, SymbolDraft, TypeScriptParser};
...
pub struct ReconcileReport {
    ...
    pub next_index: Option<TypeScriptSyntaxIndex>,
...
pub struct IndexReport {
    ...
    pub syntax_index: TypeScriptSyntaxIndex,
}

/// Latest TypeScript source text plus role/package metadata from the same scan.
#[derive(Debug, Clone, Default, PartialEq, Eq)]
pub struct TypeScriptSources {
    pub files: BTreeMap<RepoRelativePath, Arc<str>>,
    pub metadata: BTreeMap<RepoRelativePath, SourceMetadata>,
}

impl TypeScriptSources {
...
/// Failure to discover, read, parse, or validate the TypeScript syntax index.
#[derive(Debug, Error)]
pub enum TypeScriptIndexError {
    #[error(transparent)]
    Discovery(#[from] DiscoveryError),
    #[error("failed to read TypeScript source {path}: {source}")]
    Read {
        path: RepoRelativePath,
        #[source]
        source: io::Error,
    },
    #[error("TypeScript source {path} exceeds the {limit}-byte indexing budget")]
    SourceTooLarge {
        path: RepoRelativePath,
        limit: usize,
    },
    #[error("indexed TypeScript sources exceed the {limit}-byte repository budget")]
    RepositoryTooLarge { limit: usize },
    #[error("failed to parse TypeScript source: {0}")]
    Parse(String),
    #[error("failed to start a bounded TypeScript parser worker: {0}")]
    WorkerSpawn(#[source] io::Error),
    #[error("a bounded TypeScript parser worker panicked")]
    WorkerPanicked,
    #[error("TypeScript syntax index update failed: {0}")]
    Update(String),
    #[error(transparent)]
    Graph(#[from] GraphError),
    #[error("constructed syntax graph is inconsistent: {0}")]
    Consistency(#[from] ConsistencyError),
    #[error("TypeScript syntax indexing was cancelled")]
    Cancelled,
}
...
#[derive(Debug, Clone)]
pub struct TypeScriptSyntaxIndex {
...
impl Default for TypeScriptSyntaxIndex {
    fn default() -> Self {
        Self {
            files: BTreeMap::new(),
...
impl TypeScriptSyntaxIndex {
    pub fn from_sources(
        sources: BTreeMap<RepoRelativePath, Arc<str>>,
    ) -> Result<(Self, SymbolGraph), TypeScriptIndexError> {
    ...
    ) -> Result<(Self, SymbolGraph, LanguageBuildMetrics), TypeScriptIndexError> {
    ...
    pub fn reconcile_sources(
        &self,
        sources: BTreeMap<RepoRelativePath, Arc<str>>,
    ) -> Result<ReconcileReport, TypeScriptIndexError> {
    ...
    pub fn reconcile_classified_sources_bounded(
        ...
        let mut parser = TypeScriptParser::new().map_err(parse_error)?;
    ...
    fn materialize_graph_bounded(
        &self,
        cancellation: &IndexCancellation,
    ) -> Result<(SymbolGraph, GraphBuildReport), TypeScriptIndexError> {
    ...
    fn materialize_graph_delta(
        ...
        cancellation: &IndexCancellation,
    ) -> Result<Option<(SymbolGraph, GraphBuildReport)>, TypeScriptIndexError> {
        ...
        for path in changed_paths {
            ...
            if stable_symbol_paths.contains(path) {
                let file = self.files.get(path).ok_or_else(|| {
                    TypeScriptIndexError::Update(format!("missing changed file {path}"))
                })?;
                let ids: Vec<_> = graph
...
fn parse_sources_scheduled(
    ...
    parallel_file_threshold: usize,
    cancellation: &IndexCancellation,
) -> Result<(BTreeMap<RepoRelativePath, Arc<ParsedFile>>, ParseSchedule), TypeScriptIndexError> {
...
fn parse_sources_scheduled_inner(
    ...
    let outputs = thread::scope(
        |scope| -> Result<Vec<ParseWorkerOutput>, TypeScriptIndexError> {
            let mut workers = Vec::with_capacity(worker_count);
            let mut spawn_error = None;
            for worker in 0..worker_count {
                let name = format!("chakra-typescript-parser-{worker}");
            ...
            if worker_panicked {
                return Err(TypeScriptIndexError::WorkerPanicked);
            }
            if let Some(error) = spawn_error {
                return Err(TypeScriptIndexError::WorkerSpawn(error));
            }
            Ok(outputs)
        },
    )?;
    ...
    {
        return Err(TypeScriptIndexError::Parse(error));
    }
    ...
    for output in outputs {
        for (index, parsed) in output.parsed {
            let Some(slot) = ordered.get_mut(index) else {
                return Err(TypeScriptIndexError::Update(
    ...
    for ((path, _), parsed) in inputs.into_iter().zip(ordered) {
        let parsed = parsed.ok_or_else(|| {
            TypeScriptIndexError::Update("parser worker omitted an admitted source".to_owned())
        })?;
        let parsed = parsed.map_err(TypeScriptIndexError::Parse)?;
        files.insert(path, Arc::new(parsed));
    }
    Ok((
        files,
...
fn check_cancelled(cancellation: &IndexCancellation) -> Result<(), TypeScriptIndexError> {
    if cancellation.is_cancelled() {
        Err(TypeScriptIndexError::Cancelled)
    } else {
        Ok(())
    }
}
...
fn build_all_relationships(
    ...
    limit: u64,
    cancellation: &IndexCancellation,
) -> Result<BTreeMap<RepoRelativePath, Arc<RelationshipContribution>>, TypeScriptIndexError> {
...
fn parse_error(error: impl std::fmt::Display) -> TypeScriptIndexError {
    TypeScriptIndexError::Parse(error.to_string())
}
...
fn read_sources(repository_root: &Path) -> Result<TypeScriptSources, TypeScriptIndexError> {
    let files = discover_typescript_sources(repository_root)?;
    ...
    Ok(TypeScriptSources {
        files: sources,
        metadata,
    })
}

/// Builds a complete TypeScript syntax index from the actual materialized Git
...
pub fn index_repository(root: &Path) -> Result<IndexReport, TypeScriptIndexError> {
    ...
    let span = info_span!("typescript_repository_index", root = %repository_root.display());
    let _entered = span.enter();
    let sources = read_sources(&repository_root)?;
    let discovered_files = sources.files.len() as u64;
    let (syntax_index, graph) = TypeScriptSyntaxIndex::from_classified_sources(sources)?;
    ...
    info!(
        ...
        "TypeScript syntax index completed"
    );
    Ok(IndexReport {
        repository_root,
        graph,
...
/// Reads the latest Git-aware TypeScript file inventory and exact contents.
