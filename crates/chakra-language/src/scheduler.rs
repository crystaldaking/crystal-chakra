...
impl<T> PriorityWorkQueue<T> {
    ...
    pub fn push(&mut self, class: WorkClass, item: T, enqueued: Instant) -> Result<(), QueueFull> {
        ...
        if backlog.len() >= self.per_class_capacity {
            self.metrics.classes[class.index()].rejected = self.metrics.classes[class.index()]
                .rejected
                .saturating_add(1);
        ...
        self.metrics.classes[class.index()].enqueued = self.metrics.classes[class.index()]
            .enqueued
            .saturating_add(1);
        Ok(())
    }
    ...
    fn take(&mut self, class: WorkClass, position: usize) -> Option<T> {
        ...
        let counters = &mut self.metrics.classes[class.index()];
