...
use chakra_domain::project::ProjectUnitChangeKind;
...
fn manifest_change_reports_metadata_reason_without_reparse() -> Result<(), Box<dyn Error>> {
    ...
    write(
        repository.path(),
        "Cargo.toml",
        "[package]\nname = \"chakra-live-diagnostics-renamed\"\nversion = \"0.1.0\"\nedition = \"2024\"\n",
    )?;
    engine.require_fresh()?;
    ...
    let diagnostics = live.diagnostics();
    let impact = diagnostics
        .project_invalidations
        .last_impact
        .ok_or("project-scope invalidation impact missing")?;
    assert_eq!(diagnostics.project_invalidations.impacted_units, 2);
    assert_eq!(impact.changes.len(), 2);
    assert!(impact.changes.iter().any(|change| {
        change.kinds == [ProjectUnitChangeKind::Added]
            && change
                .unit
                .as_str()
                .contains("chakra-live-diagnostics-renamed")
    }));
    assert!(impact.changes.iter().any(|change| {
        change.kinds == [ProjectUnitChangeKind::Removed]
            && change.unit.as_str().contains("chakra-live-diagnostics")
    }));
    live.shutdown()?;
    Ok(())
}
...
fn status_query_carries_diagnostics_and_engine_cold_build_count() -> Result<(), Box<dyn Error>> {
    ...
    assert_eq!(
        ...
        "freshness-only publication must not count the initial graph twice"
    );
