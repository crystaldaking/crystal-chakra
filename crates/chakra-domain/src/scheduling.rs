...
/// significant: [`WorkClass::FreshnessEdit`] is the greatest (most urgent)
...
pub struct WorkQueueMetrics {
    pub freshness_edit: WorkClassQueueMetrics,
    pub provider_sync: WorkClassQueueMetrics,
    pub reconciliation: WorkClassQueueMetrics,
    pub cache_warmup: WorkClassQueueMetrics,
    pub maintenance: WorkClassQueueMetrics,
}

impl WorkQueueMetrics {
    pub fn for_class(&self, class: WorkClass) -> WorkClassQueueMetrics {
        *self.for_class_ref(class)
    }

    pub fn for_class_mut(&mut self, class: WorkClass) -> &mut WorkClassQueueMetrics {
        match class {
            WorkClass::FreshnessEdit => &mut self.freshness_edit,
            WorkClass::ProviderSync => &mut self.provider_sync,
            WorkClass::Reconciliation => &mut self.reconciliation,
            WorkClass::CacheWarmup => &mut self.cache_warmup,
            WorkClass::Maintenance => &mut self.maintenance,
        }
    }

    fn for_class_ref(&self, class: WorkClass) -> &WorkClassQueueMetrics {
        match class {
            WorkClass::FreshnessEdit => &self.freshness_edit,
            WorkClass::ProviderSync => &self.provider_sync,
            WorkClass::Reconciliation => &self.reconciliation,
            WorkClass::CacheWarmup => &self.cache_warmup,
...
mod tests {
    ...
    #[test]
    fn work_queue_metrics_serialize_with_named_priority_classes()
    -> Result<(), Box<dyn std::error::Error>> {
        ...
        assert_eq!(json["freshness_edit"]["enqueued"], 1);
        assert!(json.get("provider_sync").is_some());
        assert!(json.get("reconciliation").is_some());
        assert!(json.get("cache_warmup").is_some());
        assert!(json.get("maintenance").is_some());
        Ok(())
    }
}
