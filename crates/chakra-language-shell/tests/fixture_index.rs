//! End-to-end syntax coverage over the Shell fixture.
...
use chakra_language_shell::{IndexMetrics, index_repository};
...
#[test]
fn fixture_extracts_functions_imports_tests_and_calls() -> Result<(), Box<dyn Error>> {
    ...
    for (qualified_name, kind) in [
        (
            "src::payment_controller::refund_controller",
            SymbolKind::Function,
        ),
        ("src::payment_service::refund_service", SymbolKind::Function),
        (
            "src::payment_provider::refund_provider",
            SymbolKind::Function,
        ),
        ("src::lib::shared::record_event", SymbolKind::Function),
        (
            "tests::payment_flow_test::test_refund_flow",
            SymbolKind::Test,
        ),
    ] {
        assert!(
    ...
    assert!(graph.symbols().iter().all(|symbol| {
        symbol.key.language == Language::Shell
    ...
    assert!(
        graph
            ...
            .all(
                |call| matches!(call.resolution, CallResolution::Resolved { .. })
...
#[test]
fn fixture_roles_and_projects_come_from_shellcheck_boundaries() -> Result<(), Box<dyn Error>> {
    ...
    assert_eq!(coverage.shell_project_metadata_files, 7);

    for (path, role) in [
        ("src/payment_service.sh", SourceRole::Production),
        ("tests/payment_flow_test.sh", SourceRole::Test),
        ("vendor/external.sh", SourceRole::Vendor),
        ("generated/build_info.sh", SourceRole::Generated),
    ] {
        let metadata = graph
...
#[test]
fn malformed_shell_keeps_valid_symbols_and_reports_diagnostics() -> Result<(), Box<dyn Error>> {
    let repository = fixture_repository()?;
    fs::write(
        repository.path().join("src/payment_controller.sh"),
    ...
    assert!(diagnostics.diagnostics.iter().all(|diagnostic| {
        diagnostic.language == Language::Shell
            && diagnostic.cause == SyntaxDiagnosticCause::ParseRecovery
    }));
    assert!(
        report.graph.symbols().iter().any(|symbol| {
            symbol.key.qualified_name == "src::payment_controller::retained_marker"
        })
    );
    Ok(())
}
...
fn callers_and_incremental_reconcile_use_shell_function_edges() -> Result<(), Box<dyn Error>> {
    ...
    assert_eq!(
        names,
        BTreeSet::from(["src::payment_service::refund_service"])
    );

    let report = index_repository(repository.path())?;
    let service = repository.path().join("src/payment_service.sh");
    let source = fs::read_to_string(&service)?;
    fs::write(
        &service,
        source.replace("refund_provider", "refund_provider_changed"),
    )?;
    let sources = chakra_language_shell::scan_repository_sources(repository.path())?;
