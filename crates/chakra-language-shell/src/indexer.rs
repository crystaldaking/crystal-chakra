//! Deterministic Shell syntax indexing with reusable per-file facts.
...
use crate::discovery::{DiscoveryError, discover_shell_sources, resolve_repository_root};
use crate::parser::{ParsedFile, ShellParser, SymbolDraft};
...
pub struct ReconcileReport {
    ...
    pub next_index: Option<ShellSyntaxIndex>,
...
pub struct IndexReport {
    ...
    pub syntax_index: ShellSyntaxIndex,
}

/// Latest Shell source text plus role/package metadata from the same scan.
...
/// Failure to discover, read, parse, or validate the Shell syntax index.
#[derive(Debug, Error)]
pub enum ShellIndexError {
    #[error(transparent)]
    Discovery(#[from] DiscoveryError),
    #[error("failed to read Shell source {path}: {source}")]
    Read {
        path: RepoRelativePath,
        #[source]
        source: io::Error,
    },
    #[error("Shell source {path} exceeds the {limit}-byte indexing budget")]
    SourceTooLarge {
        path: RepoRelativePath,
        limit: usize,
    },
    #[error("indexed Shell sources exceed the {limit}-byte repository budget")]
    RepositoryTooLarge { limit: usize },
    #[error("failed to parse Shell source: {0}")]
    Parse(String),
    #[error("failed to start a bounded Shell parser worker: {0}")]
    WorkerSpawn(#[source] io::Error),
    #[error("a bounded Shell parser worker panicked")]
    WorkerPanicked,
    #[error("Shell syntax index update failed: {0}")]
    Update(String),
    #[error(transparent)]
    Graph(#[from] GraphError),
    #[error("constructed syntax graph is inconsistent: {0}")]
    Consistency(#[from] ConsistencyError),
    #[error("Shell syntax indexing was cancelled")]
    Cancelled,
}
...
impl Default for ShellSyntaxIndex {
    fn default() -> Self {
        Self {
            files: BTreeMap::new(),
...
impl ShellSyntaxIndex {
    pub fn from_sources(
        sources: BTreeMap<RepoRelativePath, Arc<str>>,
    ) -> Result<(Self, SymbolGraph), ShellIndexError> {
    ...
    ) -> Result<(Self, SymbolGraph, LanguageBuildMetrics), ShellIndexError> {
    ...
    pub fn reconcile_sources(
        &self,
        sources: BTreeMap<RepoRelativePath, Arc<str>>,
    ) -> Result<ReconcileReport, ShellIndexError> {
    ...
    pub fn reconcile_classified_sources_bounded(
        ...
        let mut parser = ShellParser::new().map_err(parse_error)?;
    ...
    fn materialize_graph_bounded(
        &self,
        cancellation: &IndexCancellation,
    ) -> Result<(SymbolGraph, GraphBuildReport), ShellIndexError> {
    ...
    fn materialize_graph_delta(
        ...
        cancellation: &IndexCancellation,
    ) -> Result<Option<(SymbolGraph, GraphBuildReport)>, ShellIndexError> {
        ...
        for path in changed_paths {
            ...
            if stable_symbol_paths.contains(path) {
                let file = self.files.get(path).ok_or_else(|| {
                    ShellIndexError::Update(format!("missing changed file {path}"))
                })?;
                let ids: Vec<_> = graph
...
fn parse_sources_scheduled(
    ...
    parallel_file_threshold: usize,
    cancellation: &IndexCancellation,
) -> Result<(BTreeMap<RepoRelativePath, Arc<ParsedFile>>, ParseSchedule), ShellIndexError> {
...
fn parse_sources_scheduled_inner(
    ...
    let outputs = thread::scope(|scope| -> Result<Vec<ParseWorkerOutput>, ShellIndexError> {
        let mut workers = Vec::with_capacity(worker_count);
        let mut spawn_error = None;
        for worker in 0..worker_count {
            let name = format!("chakra-shell-parser-{worker}");
            match thread::Builder::new().name(name).spawn_scoped(scope, || {
                ...
                let output = (|| {
                    let mut parser = match ShellParser::new() {
                        Ok(parser) => parser,
                        Err(error) => {
        ...
        if worker_panicked {
            return Err(ShellIndexError::WorkerPanicked);
        }
        if let Some(error) = spawn_error {
            return Err(ShellIndexError::WorkerSpawn(error));
        }
        Ok(outputs)
    })?;
    check_cancelled(cancellation)?;
    ...
    {
        return Err(ShellIndexError::Parse(error));
    }
    ...
    for output in outputs {
        for (index, parsed) in output.parsed {
            let Some(slot) = ordered.get_mut(index) else {
                return Err(ShellIndexError::Update(
    ...
    for ((path, _), parsed) in inputs.into_iter().zip(ordered) {
        let parsed = parsed.ok_or_else(|| {
            ShellIndexError::Update("parser worker omitted an admitted source".to_owned())
        })?;
        let parsed = parsed.map_err(ShellIndexError::Parse)?;
        files.insert(path, Arc::new(parsed));
    }
    Ok((
        files,
...
fn check_cancelled(cancellation: &IndexCancellation) -> Result<(), ShellIndexError> {
    if cancellation.is_cancelled() {
        Err(ShellIndexError::Cancelled)
    } else {
        Ok(())
    }
}
...
fn build_all_relationships(
    ...
    limit: u64,
    cancellation: &IndexCancellation,
) -> Result<BTreeMap<RepoRelativePath, Arc<RelationshipContribution>>, ShellIndexError> {
...
fn parse_error(error: impl std::fmt::Display) -> ShellIndexError {
    ShellIndexError::Parse(error.to_string())
}
...
fn read_sources(repository_root: &Path) -> Result<ShellSources, ShellIndexError> {
    let files = discover_shell_sources(repository_root)?;
    ...
    for discovered in files {
        ...
        file.take((MAX_SOURCE_FILE_BYTES + 1) as u64)
            ...
            .map_err(|source| ShellIndexError::Read {
                path: path.clone(),
                source,
            })?;
        if source.len() > MAX_SOURCE_FILE_BYTES {
            return Err(ShellIndexError::SourceTooLarge {
                path,
        ...
        total_bytes =
            total_bytes
                ...
                .ok_or(ShellIndexError::RepositoryTooLarge {
                    limit: MAX_REPOSITORY_SOURCE_BYTES,
                })?;
        if total_bytes > MAX_REPOSITORY_SOURCE_BYTES {
            return Err(ShellIndexError::RepositoryTooLarge {
                limit: MAX_REPOSITORY_SOURCE_BYTES,
            });
        }
...
/// Builds a complete Shell syntax index from the actual materialized Git
/// worktree. The caller owns atomic publication into `WorkspaceEngine`.
pub fn index_repository(root: &Path) -> Result<IndexReport, ShellIndexError> {
    let started = Instant::now();
    let repository_root = resolve_repository_root(root)?;
    let span = info_span!("shell_repository_index", root = %repository_root.display());
    let _entered = span.enter();
    let sources = read_sources(&repository_root)?;
    let discovered_files = sources.files.len() as u64;
    let (syntax_index, graph) = ShellSyntaxIndex::from_classified_sources(sources)?;
...
/// Reads the latest Git-aware Shell file inventory and exact contents.
pub fn scan_repository_sources(repository_root: &Path) -> Result<ShellSources, ShellIndexError> {
...
mod tests {
    ...
    fn bounded_parallel_parsing_is_deterministic() -> Result<(), Box<dyn Error>> {
        ...
        for index in 0..64 {
            sources.insert(
                RepoRelativePath::new(format!("src/generated_{index:03}.sh"))?,
                Arc::<str>::from(format!(
                    "caller_{index}() {{ helper_{index}; }}\nhelper_{index}() {{ true; }}\n"
                )),
            );
        }
        let cancellation = IndexCancellation::default();
        let (_, sequential, sequential_metrics) = ShellSyntaxIndex::from_sources_scheduled(
            sources.clone(),
            GraphBuildLimits::UNLIMITED,
            1,
            1,
        ...
        let (_, parallel, parallel_metrics) = ShellSyntaxIndex::from_sources_scheduled(
            sources,
            GraphBuildLimits::UNLIMITED,
            4,
            1,
    ...
    fn cancelling_parallel_parse_joins_every_scoped_worker() -> Result<(), Box<dyn Error>> {
        ...
        for index in 0..64 {
            sources.insert(
                RepoRelativePath::new(format!("src/cancel_{index:03}.sh"))?,
                Arc::<str>::from(format!("item_{index}() {{ true; }}\n")),
            );
        }
        let cancellation = IndexCancellation::default();
        ...
        assert!(matches!(result, Err(ShellIndexError::Cancelled)));
        Ok(())
    }
    ...
    fn duplicate_call_fanout_is_stored_linearly() -> Result<(), Box<dyn Error>> {
        ...
        for index in 0..TARGETS {
            sources.insert(
                RepoRelativePath::new(format!("src/target_{index:03}.sh"))?,
                Arc::<str>::from("target() { true; }\n"),
            );
        }
        for index in 0..CALLS {
            sources.insert(
                RepoRelativePath::new(format!("src/caller_{index:03}.sh"))?,
                Arc::<str>::from(format!("caller_{index}() {{ target; }}\n")),
            );
        }

        let started = Instant::now();
        let (_, graph) = ShellSyntaxIndex::from_sources(sources)?;
    ...
    fn truncated_catalog_never_turns_ambiguity_into_a_unique_call() -> Result<(), Box<dyn Error>> {
        let mut sources = BTreeMap::new();
        sources.insert(
            RepoRelativePath::new("src/a_caller.sh")?,
            Arc::<str>::from("invoke() { target; }\n"),
        );
        sources.insert(
            RepoRelativePath::new("src/b_target.sh")?,
            Arc::<str>::from("target() { true; }\n"),
        );
        sources.insert(
            RepoRelativePath::new("src/c_target.sh")?,
            Arc::<str>::from("target() { true; }\n"),
        );
        let cancellation = IndexCancellation::default();
        let (_, graph, metrics) = ShellSyntaxIndex::from_sources_bounded(
            sources,
            GraphBuildLimits {
                max_symbols: 2,
                max_edges: 10,
        ...
        // Each Shell file declares two symbols (the script module plus its function):
    ...
    fn metadata_change_republishes_without_reparsing_source() -> Result<(), Box<dyn Error>> {
        let path = RepoRelativePath::new("src/lib.sh")?;
        let source = Arc::<str>::from("stable() { true; }\n");
        let initial = ShellSources {
            files: BTreeMap::from([(path.clone(), source.clone())]),
            metadata: BTreeMap::from([(path.clone(), SourceMetadata::path_fallback(&path))]),
        };
        let (index, _) = ShellSyntaxIndex::from_classified_sources(initial)?;
        let changed = ShellSources {
            ...
            metadata: BTreeMap::from([(
                ...
                SourceMetadata {
                    ...
                    classification:
                        chakra_domain::source::SourceClassification::ShellProjectMetadata,
    ...
    fn rejects_a_source_larger_than_the_file_budget() -> Result<(), Box<dyn Error>> {
        ...
        let file = File::create(repository.path().join("src/large.sh"))?;
        ...
        assert!(matches!(
            error,
            ShellIndexError::SourceTooLarge { limit, .. } if limit == MAX_SOURCE_FILE_BYTES
        ));
        Ok(())
    }
}
