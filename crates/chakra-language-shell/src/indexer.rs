//! Deterministic shell syntax indexing on the shared language-neutral driver
...
use crate::discovery::discover_shell_sources;
...
pub type ShellIndexError = chakra_language_index::LanguageIndexError;
/// Latest source text plus role/package metadata from the same scan.
pub type ShellSources = chakra_language_index::LanguageSources;
/// Reusable per-file syntax facts and per-owner relationship contributions.
pub type ShellSyntaxIndex = chakra_language_index::LanguageSyntaxIndex<ShellHooks>;
/// Complete private initial index, ready for atomic publication.
pub type IndexReport = chakra_language_index::IndexReport<ShellHooks>;
/// Reconcile outcome for the syntax index.
pub type ReconcileReport = chakra_language_index::ReconcileReport<ShellHooks>;

/// Language seams of the shared indexing driver.
#[derive(Debug, Clone, Copy, Default)]
pub struct ShellHooks;

impl LanguageHooks for ShellHooks {
    type Parser = ShellParser;
    ...
    fn discover_sources(root: &Path) -> Result<Vec<ClassifiedSource>, ShellIndexError> {
        Ok(discover_shell_sources(root)?)
    }
}
...
impl LanguageParser for ShellParser {
    fn parse(
        ...
        source: Arc<str>,
    ) -> Result<chakra_language_index::ParsedFile, ShellIndexError> {
        ShellParser::parse(self, path, source)
            .map_err(|error| ShellIndexError::Parse(error.to_string()))
    }
}
...
pub fn index_repository(root: &Path) -> Result<IndexReport, ShellIndexError> {
    chakra_language_index::index_repository::<ShellHooks>(root)
}
...
pub fn scan_repository_sources(repository_root: &Path) -> Result<ShellSources, ShellIndexError> {
    chakra_language_index::scan_repository_sources::<ShellHooks>(repository_root)
}
...
mod tests {
    ...
    fn cancelling_parallel_parse_joins_every_scoped_worker() -> Result<(), Box<dyn Error>> {
        ...
        let indexing = std::thread::spawn(move || {
            ...
            parse_sources_scheduled_observed::<ShellHooks>(
                sources,
                WORKERS,
                1,
                &worker_cancellation,
