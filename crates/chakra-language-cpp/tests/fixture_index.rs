//! End-to-end syntax coverage over the Cpp fixture.
...
use chakra_language_cpp::{IndexMetrics, index_repository};
...
#[test]
fn fixture_extracts_namespaces_types_includes_tests_and_calls() -> Result<(), Box<dyn Error>> {
    ...
    for (qualified_name, kind) in [
        ("chakra::payments::PaymentProvider", SymbolKind::Class),
        (
            "chakra::payments::PaymentService::refund",
            SymbolKind::Method,
        ),
        (
            "chakra::payments::PaymentController::handle_refund",
            SymbolKind::Method,
        ),
        ("chakra::payments::provider_refund", SymbolKind::Function),
        ("test_refund_flow", SymbolKind::Test),
    ] {
        assert!(
    ...
    assert!(graph.symbols().iter().all(|symbol| {
        symbol.key.language == Language::Cpp
...
#[test]
fn fixture_roles_and_projects_come_from_cmake_boundaries() -> Result<(), Box<dyn Error>> {
    ...
    assert_eq!(coverage.cpp_project_metadata_files, 7);

    for (path, role) in [
        ("src/payment_service.cpp", SourceRole::Production),
        ("tests/payment_flow_test.cpp", SourceRole::Test),
        ("vendor/external.cpp", SourceRole::Vendor),
        ("generated/payment.pb.cc", SourceRole::Generated),
    ] {
        let metadata = graph
...
#[test]
fn malformed_cpp_keeps_valid_symbols_and_reports_diagnostics() -> Result<(), Box<dyn Error>> {
    let repository = fixture_repository()?;
    fs::write(
        repository.path().join("src/payment_controller.cpp"),
    ...
    assert!(diagnostics.diagnostics.iter().all(|diagnostic| {
        diagnostic.language == Language::Cpp
    ...
    assert!(
        report
            ...
            .symbols()
            .iter()
            .any(|symbol| { symbol.key.qualified_name == "retained_marker" })
    );
    Ok(())
}
...
fn callers_and_incremental_reconcile_use_cpp_function_edges() -> Result<(), Box<dyn Error>> {
    ...
    let names: BTreeSet<_> = callers
        ...
        .collect();
    assert_eq!(names, BTreeSet::from(["chakra::payments::service_refund"]));

    let report = index_repository(repository.path())?;
    let service = repository.path().join("src/payment_service.cpp");
    let source = fs::read_to_string(&service)?;
    fs::write(
        &service,
        source.replace("payment_service_marker", "payment_service_marker_changed"),
    )?;
    let sources = chakra_language_cpp::scan_repository_sources(repository.path())?;
