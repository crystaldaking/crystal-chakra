//! Deterministic Cpp syntax indexing with reusable per-file facts.
...
use crate::discovery::{DiscoveryError, discover_cpp_sources, resolve_repository_root};
use crate::parser::{CppParser, ParsedFile, SymbolDraft};
...
pub struct ReconcileReport {
    ...
    pub next_index: Option<CppSyntaxIndex>,
...
pub struct IndexReport {
    ...
    pub syntax_index: CppSyntaxIndex,
}

/// Latest Cpp source text plus role/package metadata from the same scan.
#[derive(Debug, Clone, Default, PartialEq, Eq)]
pub struct CppSources {
...
/// Failure to discover, read, parse, or validate the Cpp syntax index.
#[derive(Debug, Error)]
pub enum CppIndexError {
    #[error(transparent)]
    Discovery(#[from] DiscoveryError),
    #[error("failed to read Cpp source {path}: {source}")]
    Read {
        path: RepoRelativePath,
        #[source]
        source: io::Error,
    },
    #[error("Cpp source {path} exceeds the {limit}-byte indexing budget")]
    SourceTooLarge {
        path: RepoRelativePath,
        limit: usize,
    },
    #[error("indexed Cpp sources exceed the {limit}-byte repository budget")]
    RepositoryTooLarge { limit: usize },
    #[error("failed to parse Cpp source: {0}")]
    Parse(String),
    #[error("failed to start a bounded Cpp parser worker: {0}")]
    WorkerSpawn(#[source] io::Error),
    #[error("a bounded Cpp parser worker panicked")]
    WorkerPanicked,
    #[error("Cpp syntax index update failed: {0}")]
    Update(String),
    #[error(transparent)]
    Graph(#[from] GraphError),
    #[error("constructed syntax graph is inconsistent: {0}")]
    Consistency(#[from] ConsistencyError),
    #[error("Cpp syntax indexing was cancelled")]
    Cancelled,
}
...
impl Default for CppSyntaxIndex {
    fn default() -> Self {
        Self {
            files: BTreeMap::new(),
...
impl CppSyntaxIndex {
    ...
    ) -> Result<(Self, SymbolGraph), CppIndexError> {
    ...
    ) -> Result<(Self, SymbolGraph, LanguageBuildMetrics), CppIndexError> {
    ...
    pub fn reconcile_sources(
        &self,
        sources: BTreeMap<RepoRelativePath, Arc<str>>,
    ) -> Result<ReconcileReport, CppIndexError> {
    ...
    pub fn reconcile_classified_sources_bounded(
        ...
        let mut parser = CppParser::new().map_err(parse_error)?;
    ...
    fn materialize_graph_bounded(
        &self,
        cancellation: &IndexCancellation,
    ) -> Result<(SymbolGraph, GraphBuildReport), CppIndexError> {
    ...
    fn materialize_graph_delta(
        ...
        cancellation: &IndexCancellation,
    ) -> Result<Option<(SymbolGraph, GraphBuildReport)>, CppIndexError> {
        ...
        for path in changed_paths {
            ...
            if stable_symbol_paths.contains(path) {
                let file = self
                    .files
                    .get(path)
                    .ok_or_else(|| CppIndexError::Update(format!("missing changed file {path}")))?;
                let ids: Vec<_> = graph
...
fn parse_sources_scheduled(
    ...
    parallel_file_threshold: usize,
    cancellation: &IndexCancellation,
) -> Result<(BTreeMap<RepoRelativePath, Arc<ParsedFile>>, ParseSchedule), CppIndexError> {
...
fn parse_sources_scheduled_inner(
    ...
    let outputs = thread::scope(|scope| -> Result<Vec<ParseWorkerOutput>, CppIndexError> {
        let mut workers = Vec::with_capacity(worker_count);
        let mut spawn_error = None;
        for worker in 0..worker_count {
            let name = format!("chakra-cpp-parser-{worker}");
        ...
        if worker_panicked {
            return Err(CppIndexError::WorkerPanicked);
        }
        if let Some(error) = spawn_error {
            return Err(CppIndexError::WorkerSpawn(error));
        }
        Ok(outputs)
    })?;
    check_cancelled(cancellation)?;
    ...
    {
        return Err(CppIndexError::Parse(error));
    }
    ...
    for output in outputs {
        for (index, parsed) in output.parsed {
            let Some(slot) = ordered.get_mut(index) else {
                return Err(CppIndexError::Update(
    ...
    for ((path, _), parsed) in inputs.into_iter().zip(ordered) {
        let parsed = parsed.ok_or_else(|| {
            CppIndexError::Update("parser worker omitted an admitted source".to_owned())
        })?;
        let parsed = parsed.map_err(CppIndexError::Parse)?;
        files.insert(path, Arc::new(parsed));
    }
    Ok((
        files,
...
fn check_cancelled(cancellation: &IndexCancellation) -> Result<(), CppIndexError> {
    if cancellation.is_cancelled() {
        Err(CppIndexError::Cancelled)
    } else {
        Ok(())
    }
}
...
fn build_all_relationships(
    ...
    limit: u64,
    cancellation: &IndexCancellation,
) -> Result<BTreeMap<RepoRelativePath, Arc<RelationshipContribution>>, CppIndexError> {
...
fn parse_error(error: impl std::fmt::Display) -> CppIndexError {
    CppIndexError::Parse(error.to_string())
}
...
fn read_sources(repository_root: &Path) -> Result<CppSources, CppIndexError> {
    let files = discover_cpp_sources(repository_root)?;
    ...
    for discovered in files {
        ...
        file.take((MAX_SOURCE_FILE_BYTES + 1) as u64)
            ...
            .map_err(|source| CppIndexError::Read {
                path: path.clone(),
                source,
            })?;
        if source.len() > MAX_SOURCE_FILE_BYTES {
            return Err(CppIndexError::SourceTooLarge {
                path,
        ...
        total_bytes =
            total_bytes
                ...
                .ok_or(CppIndexError::RepositoryTooLarge {
                    limit: MAX_REPOSITORY_SOURCE_BYTES,
                })?;
        if total_bytes > MAX_REPOSITORY_SOURCE_BYTES {
            return Err(CppIndexError::RepositoryTooLarge {
                limit: MAX_REPOSITORY_SOURCE_BYTES,
            });
        }
...
/// Builds a complete Cpp syntax index from the actual materialized Git
...
pub fn index_repository(root: &Path) -> Result<IndexReport, CppIndexError> {
    ...
    let span = info_span!("cpp_repository_index", root = %repository_root.display());
    let _entered = span.enter();
    let sources = read_sources(&repository_root)?;
    let discovered_files = sources.files.len() as u64;
    let (syntax_index, graph) = CppSyntaxIndex::from_classified_sources(sources)?;
...
/// Reads the latest Git-aware Cpp file inventory and exact contents.
