...
//!
//! pyright has no rust-analyzer-style `experimental/serverStatus` quiescence
...
use crate::{Command, PyrightConfig, SharedState};
...
pub(crate) enum ProviderError {
    #[error("pyright transport failed: {0}")]
    Transport(String),
    #[error("timed out waiting for the pyright response")]
    Timeout,
    #[error("the pyright request was cancelled by its caller")]
    Cancelled,
    #[error("pyright request failed ({code}): {message}")]
    Server { code: i32, message: String },
    #[error("invalid pyright response: {0}")]
    InvalidResponse(#[from] serde_json::Error),
    #[error("pyright does not advertise call hierarchy")]
    Unsupported,
...
impl Worker {
    pub(crate) fn new(
        ...
        let (workspace_documents, workspace_source_bytes) =
            initial_workspace.document_stats(Language::Python);
    ...
    fn query_with_owned_session(
        ...
        let mut session = self
            .session
            .take()
            .ok_or_else(|| ProviderError::Transport("no active pyright process".to_owned()))?;
    ...
    fn synchronize_documents(
        ...
        let target_document = workspace
            ...
            .filter(|document| document.language == Language::Python)
            .ok_or(ProviderError::InvalidPosition)?;
    ...
    fn start_session(&mut self) -> Result<(), ProviderError> {
        self.set_progress(ProviderProgress {
            ...
            source: ProviderProgressSource::Chakra,
            message: Some("starting the pyright process".to_owned()),
            percentage: None,
        });
    ...
    fn drain_session_events(&mut self) {
        ...
        if closed {
            ...
            self.set_state(
                ProviderState::Degraded,
                None,
                Some("pyright process closed its output".to_owned()),
            );
        } else {
            self.session = Some(session);
        }
...
fn language_id(_path: &RepoRelativePath) -> &'static str {
    // pyright serves `.py` and `.pyi` sources under the same language id.
    "python"
}
...
mod tests {
    ...
    fn language_ids_follow_the_file_extension() -> Result<(), Box<dyn std::error::Error>> {
        assert_eq!(language_id(&RepoRelativePath::new("src/a.py")?), "python");
        assert_eq!(language_id(&RepoRelativePath::new("src/a.pyi")?), "python");
        Ok(())
    }
}
