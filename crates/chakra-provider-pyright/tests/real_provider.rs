...
//! suite never depends on a developer-global pyright installation.
...
#[test]
#[ignore = "requires pyright on PATH or CHAKRA_PYRIGHT"]
fn current_pyright_returns_precise_incoming_calls_across_revisions() -> Result<(), Box<dyn Error>> {
    let repository = tempfile::tempdir()?;
    fs::write(
        repository.path().join("pyrightconfig.json"),
        "{\"include\":[\"main.py\"]}\n",
    )?;
    let path = RepoRelativePath::new("main.py")?;
    let source: Arc<str> = Arc::from("def target():\n    pass\n\ndef caller():\n    target()\n");
    fs::write(repository.path().join(path.as_str()), source.as_ref())?;
    let initial = request(repository.path(), path.clone(), source, Revision(1))?;
    let command = std::env::var_os("CHAKRA_PYRIGHT")
        .map_or_else(PyrightCommand::discover, |path| {
            Some(PyrightCommand::stdio(path))
        })
        .ok_or("pyright not found")?;
    ...
    assert!(
        result
            .incoming
            .iter()
            .all(|relation| relation.provenance == Provenance::Pyright)
    );
    ...
    eprintln!(
        "pyright_enrichment: initial={initial_elapsed:?}, after_edit={changed_elapsed:?}, initial_incoming={}, changed_incoming={}",
        result.incoming.len(),
