//! Deterministic Python syntax indexing with reusable per-file facts.
...
use crate::discovery::{DiscoveryError, discover_python_sources, resolve_repository_root};
use crate::parser::{ParsedFile, PythonParser, SymbolDraft};
...
pub struct ReconcileReport {
    ...
    pub next_index: Option<PythonSyntaxIndex>,
...
pub struct IndexReport {
    ...
    pub syntax_index: PythonSyntaxIndex,
}

/// Latest Python source text plus role/package metadata from the same scan.
...
/// Failure to discover, read, parse, or validate the Python syntax index.
#[derive(Debug, Error)]
pub enum PythonIndexError {
    #[error(transparent)]
    Discovery(#[from] DiscoveryError),
    #[error("failed to read Python source {path}: {source}")]
    Read {
        path: RepoRelativePath,
        #[source]
        source: io::Error,
    },
    #[error("Python source {path} exceeds the {limit}-byte indexing budget")]
    SourceTooLarge {
        path: RepoRelativePath,
        limit: usize,
    },
    #[error("indexed Python sources exceed the {limit}-byte repository budget")]
    RepositoryTooLarge { limit: usize },
    #[error("failed to parse Python source: {0}")]
    Parse(String),
    #[error("failed to start a bounded Python parser worker: {0}")]
    WorkerSpawn(#[source] io::Error),
    #[error("a bounded Python parser worker panicked")]
    WorkerPanicked,
    #[error("Python syntax index update failed: {0}")]
    Update(String),
    #[error(transparent)]
    Graph(#[from] GraphError),
    #[error("constructed syntax graph is inconsistent: {0}")]
    Consistency(#[from] ConsistencyError),
    #[error("Python syntax indexing was cancelled")]
    Cancelled,
}
...
impl Default for PythonSyntaxIndex {
    fn default() -> Self {
        Self {
            files: BTreeMap::new(),
...
impl PythonSyntaxIndex {
    ...
    ) -> Result<(Self, SymbolGraph), PythonIndexError> {
    ...
    ) -> Result<(Self, SymbolGraph, LanguageBuildMetrics), PythonIndexError> {
    ...
    pub fn reconcile_sources(
        &self,
        sources: BTreeMap<RepoRelativePath, Arc<str>>,
    ) -> Result<ReconcileReport, PythonIndexError> {
    ...
    pub fn reconcile_classified_sources_bounded(
        ...
        let mut parser = PythonParser::new().map_err(parse_error)?;
    ...
    fn materialize_graph_bounded(
        &self,
        cancellation: &IndexCancellation,
    ) -> Result<(SymbolGraph, GraphBuildReport), PythonIndexError> {
    ...
    fn materialize_graph_delta(
        ...
        cancellation: &IndexCancellation,
    ) -> Result<Option<(SymbolGraph, GraphBuildReport)>, PythonIndexError> {
        ...
        for path in changed_paths {
            ...
            if stable_symbol_paths.contains(path) {
                let file = self.files.get(path).ok_or_else(|| {
                    PythonIndexError::Update(format!("missing changed file {path}"))
                })?;
                let ids: Vec<_> = graph
...
fn parse_sources_scheduled(
    ...
    parallel_file_threshold: usize,
    cancellation: &IndexCancellation,
) -> Result<(BTreeMap<RepoRelativePath, Arc<ParsedFile>>, ParseSchedule), PythonIndexError> {
...
fn parse_sources_scheduled_inner(
    ...
    let outputs = thread::scope(
        |scope| -> Result<Vec<ParseWorkerOutput>, PythonIndexError> {
            let mut workers = Vec::with_capacity(worker_count);
            let mut spawn_error = None;
            for worker in 0..worker_count {
                let name = format!("chakra-python-parser-{worker}");
            ...
            if worker_panicked {
                return Err(PythonIndexError::WorkerPanicked);
            }
            if let Some(error) = spawn_error {
                return Err(PythonIndexError::WorkerSpawn(error));
            }
            Ok(outputs)
        },
    )?;
    ...
    {
        return Err(PythonIndexError::Parse(error));
    }
    ...
    for output in outputs {
        for (index, parsed) in output.parsed {
            let Some(slot) = ordered.get_mut(index) else {
                return Err(PythonIndexError::Update(
    ...
    for ((path, _), parsed) in inputs.into_iter().zip(ordered) {
        let parsed = parsed.ok_or_else(|| {
            PythonIndexError::Update("parser worker omitted an admitted source".to_owned())
        })?;
        let parsed = parsed.map_err(PythonIndexError::Parse)?;
        files.insert(path, Arc::new(parsed));
    }
    Ok((
        files,
...
fn check_cancelled(cancellation: &IndexCancellation) -> Result<(), PythonIndexError> {
    if cancellation.is_cancelled() {
        Err(PythonIndexError::Cancelled)
    } else {
        Ok(())
    }
}
...
fn build_all_relationships(
    ...
    limit: u64,
    cancellation: &IndexCancellation,
) -> Result<BTreeMap<RepoRelativePath, Arc<RelationshipContribution>>, PythonIndexError> {
...
fn parse_error(error: impl std::fmt::Display) -> PythonIndexError {
    PythonIndexError::Parse(error.to_string())
}
...
fn read_sources(repository_root: &Path) -> Result<PythonSources, PythonIndexError> {
    let files = discover_python_sources(repository_root)?;
    ...
    for discovered in files {
        ...
        file.take((MAX_SOURCE_FILE_BYTES + 1) as u64)
            ...
            .map_err(|source| PythonIndexError::Read {
                path: path.clone(),
                source,
            })?;
        if source.len() > MAX_SOURCE_FILE_BYTES {
            return Err(PythonIndexError::SourceTooLarge {
                path,
        ...
        total_bytes =
            total_bytes
                ...
                .ok_or(PythonIndexError::RepositoryTooLarge {
                    limit: MAX_REPOSITORY_SOURCE_BYTES,
                })?;
        if total_bytes > MAX_REPOSITORY_SOURCE_BYTES {
            return Err(PythonIndexError::RepositoryTooLarge {
                limit: MAX_REPOSITORY_SOURCE_BYTES,
            });
        }
...
/// Builds a complete Python syntax index from the actual materialized Git
...
pub fn index_repository(root: &Path) -> Result<IndexReport, PythonIndexError> {
    ...
    let span = info_span!("python_repository_index", root = %repository_root.display());
    let _entered = span.enter();
    let sources = read_sources(&repository_root)?;
    let discovered_files = sources.files.len() as u64;
    let (syntax_index, graph) = PythonSyntaxIndex::from_classified_sources(sources)?;
...
/// Reads the latest Git-aware Python file inventory and exact contents.
