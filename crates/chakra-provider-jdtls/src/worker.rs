...
//!
//! jdtls has no rust-analyzer-style `experimental/serverStatus` quiescence
//! signal. Readiness is instead proven by a post-synchronization request
//! barrier (ADR-0032/0035): after the current document generation is sent,
//! the first `textDocument/prepareCallHierarchy` round-trip confirms the
//! server consumed it. Because jdtls imports the project model before
//! answering honestly — a cold data directory can take minutes — the prepare
//! round-trips are bounded by the configurable `readiness_timeout` rather
//! than the per-request timeout. An empty prepare after the barrier is a
//! genuine "no item", not a reason to wait forever.
...
use crate::{Command, JdtlsConfig, SharedState};
...
pub(crate) enum ProviderError {
    #[error("jdtls transport failed: {0}")]
    Transport(String),
    #[error("timed out waiting for the jdtls response")]
    Timeout,
    #[error("the jdtls request was cancelled by its caller")]
    Cancelled,
    #[error("jdtls request failed ({code}): {message}")]
    Server { code: i32, message: String },
    #[error("invalid jdtls response: {0}")]
    InvalidResponse(#[from] serde_json::Error),
    #[error("jdtls does not advertise call hierarchy")]
    Unsupported,
...
impl Worker {
    pub(crate) fn new(
        ...
        let (workspace_documents, workspace_source_bytes) =
            initial_workspace.document_stats(Language::Java);
    ...
    fn query_with_owned_session(
        ...
        let mut session = self
            .session
            .take()
            .ok_or_else(|| ProviderError::Transport("no active jdtls process".to_owned()))?;
    ...
    fn query_session(
        ...
        // The readiness barrier (prepare round-trips) gets the generous,
        // configurable readiness bound: jdtls may still be importing the
        // project model while the first queries arrive.
        let readiness_deadline = self.operation_deadline(self.config.readiness_timeout);
        let mut last_incoming = Vec::new();
        let mut last_outgoing = Vec::new();

        for attempt in 0..2 {
            self.check_operation()?;
            let items = self.prepare_call_hierarchy(session, request, readiness_deadline)?;
            ...
            // Readiness may legitimately consume most of its dedicated
            // cold-import bound. Give the actual hierarchy calls their own
            // request-sized deadline instead of reusing the pre-barrier one.
            let hierarchy_deadline = self.operation_deadline(self.config.request_timeout);
    ...
    fn synchronize_documents(
        ...
        let target_document = workspace
            ...
            .filter(|document| document.language == Language::Java)
            .ok_or(ProviderError::InvalidPosition)?;
    ...
    fn start_session(&mut self) -> Result<(), ProviderError> {
        self.set_progress(ProviderProgress {
            ...
            source: ProviderProgressSource::Chakra,
            message: Some("starting the jdtls process".to_owned()),
            percentage: None,
        });
    ...
    fn drain_session_events(&mut self) {
        ...
        if closed {
            ...
            self.set_state(
                ProviderState::Degraded,
                None,
                Some("jdtls process closed its output".to_owned()),
            );
        } else {
            self.session = Some(session);
        }
...
fn language_id(_path: &RepoRelativePath) -> &'static str {
    // jdtls serves every `.java` source under one language id.
    "java"
}
...
mod tests {
    ...
    fn language_ids_follow_the_file_extension() -> Result<(), Box<dyn std::error::Error>> {
        assert_eq!(language_id(&RepoRelativePath::new("src/A.java")?), "java");
        Ok(())
    }
}
