...
//! syntax snapshot, and every converted fact carries `Provenance::Jdtls`.
...
fn identifier_boundary(line: &str, byte: usize, length: usize) -> bool {
    ...
    !before.is_some_and(is_java_identifier_character)
        && !after.is_some_and(is_java_identifier_character)
}

fn is_java_identifier_character(character: char) -> bool {
...
mod tests {
    ...
    fn java_document(path: &RepoRelativePath, source: &str) -> ProviderDocument {
        ProviderDocument {
            path: path.clone(),
            source: Arc::from(source),
            language: chakra_domain::symbol::Language::Java,
        }
    }
    ...
    #[test]
    fn incoming_relations_carry_jdtls_provenance_and_bounded_call_sites()
    -> Result<(), Box<dyn Error>> {
        ...
        let workspace = ProviderWorkspace::from_documents(
            ...
            Revision(4),
            vec![java_document(&path, "class Main { void target() {} }\n")],
        );
        ...
        assert_eq!(converted[0].provenance, Provenance::Jdtls);
        assert!(!truncated);
        Ok(())
    }
    ...
    fn relations_outside_the_workspace_are_dropped_before_the_limit() -> Result<(), Box<dyn Error>>
    {
        ...
        let workspace = ProviderWorkspace::from_documents(
            repository.path().to_path_buf(),
            Revision(4),
            vec![java_document(&path, "class Inside { void inside() {} }\n")],
        );
    ...
    fn unicode_positions_round_trip_through_lsp_utf16() -> Result<(), Box<dyn Error>> {
        ...
        // Column 9 is the 🦀: the "var café" prefix is 8 UTF-16 units.
    ...
    #[test]
    fn symbol_position_does_not_match_inside_a_dollar_identifier() -> Result<(), Box<dyn Error>> {
        let source = "class Main { int $target; void target() {} }\n";
        let declaration = SourceRange::new(
            RepoRelativePath::new("src/Main.java")?,
            TextPosition::new(1, 1)?,
            TextPosition::new(1, u32::try_from(source.trim_end().chars().count() + 1)?)?,
        )?;
        let position = find_symbol_position(source, "target", &declaration)?;
        let expected = source
            .find("void target")
            .ok_or("standalone target missing")?
            + "void ".len();
        assert_eq!(position, Position::new(0, u32::try_from(expected)?));
        Ok(())
    }
