//! Hermetic jdtls lifecycle regressions using a tiny scripted stdio LSP peer.
...
use chakra_provider_jdtls::{JdtlsCommand, JdtlsConfig, JdtlsProvider, workspace_data_dir};
...
fn main() -> io::Result<()> {
    ...
    let slow_ready = stem_contains("slow-ready");
...
fn compile_fake_server(root: &Path, name: &str) -> Result<PathBuf, Box<dyn Error>> {
    ...
    let status = Command::new(rustc)
        ...
        .status()?;
    if !status.success() {
        return Err("failed to compile the fake jdtls peer".into());
    }
    Ok(executable)
}
...
fn request(root: &Path, revision: Revision) -> Result<PreciseQueryRequest, Box<dyn Error>> {
    let path = RepoRelativePath::new("src/Main.java")?;
...
fn config(executable: &Path) -> JdtlsConfig {
    let repository_root = executable.parent().unwrap_or_else(|| Path::new("."));
    JdtlsConfig {
...
fn precise_incoming_call_carries_jdtls_provenance() -> Result<(), Box<dyn Error>> {
    ...
    assert_eq!(relation.provenance, Provenance::Jdtls);
...
fn revision_delta_syncs_only_opened_documents_with_full_text() -> Result<(), Box<dyn Error>> {
    ...
    request.workspace = workspace(
        ...
        Revision(1),
        vec![
            document(&RepoRelativePath::new("src/Main.java")?, TARGET_SOURCE),
            document(&other_path, "class Other {}\n"),
        ],
    )?;
    let provider = JdtlsProvider::start(request.workspace.clone(), config(&executable))?;
    ...
    let revision_two = PreciseQueryRequest {
        workspace: workspace(
            ...
            vec![
                document(&RepoRelativePath::new("src/Main.java")?, TARGET_SOURCE),
                document(&other_path, "class OtherV2 {}\n"),
            ],
        )?,
        ..request.clone()
    };
...
fn per_query_wait_budget_returns_before_the_request_timeout() -> Result<(), Box<dyn Error>> {
    ...
    let provider = JdtlsProvider::start(request.workspace.clone(), bounded)?;
...
fn cold_readiness_continues_after_the_query_wait_budget() -> Result<(), Box<dyn Error>> {
    ...
    bounded.readiness_timeout = Duration::from_secs(2);
    let provider = JdtlsProvider::start(request.workspace.clone(), bounded)?;
    ...
    // below the 2s readiness timeout for heavily loaded hosts.
    assert!(
        started.elapsed() < Duration::from_millis(1500),
        "query wait was not bounded: {:?}",
        started.elapsed()
    );

    // The lifecycle tests compile several fake servers in parallel, so give
    // process startup room while keeping the readiness assertion bounded.
    let deadline = Instant::now() + Duration::from_secs(10);
    while provider.state_for(Revision(1)) != ProviderState::Ready && Instant::now() < deadline {
        std::thread::sleep(Duration::from_millis(20));
    }
    assert_eq!(
        provider.state_for(Revision(1)),
        ProviderState::Ready,
        "the owner did not complete readiness after the caller returned; last_error={:?}",
        provider.last_error()
    );
    provider.shutdown()?;
    Ok(())
