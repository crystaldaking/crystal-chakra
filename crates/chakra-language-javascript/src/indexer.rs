//! Deterministic javascript syntax indexing on the shared language-neutral driver
...
use crate::discovery::discover_javascript_sources;
use crate::parser::JavaScriptParser;
...
pub type JavaScriptIndexError = chakra_language_index::LanguageIndexError;
/// Latest source text plus role/package metadata from the same scan.
pub type JavaScriptSources = chakra_language_index::LanguageSources;
/// Reusable per-file syntax facts and per-owner relationship contributions.
pub type JavaScriptSyntaxIndex = chakra_language_index::LanguageSyntaxIndex<JavaScriptHooks>;
/// Complete private initial index, ready for atomic publication.
pub type IndexReport = chakra_language_index::IndexReport<JavaScriptHooks>;
/// Reconcile outcome for the syntax index.
pub type ReconcileReport = chakra_language_index::ReconcileReport<JavaScriptHooks>;
...
impl LanguageHooks for JavaScriptHooks {
    ...
    fn discover_sources(root: &Path) -> Result<Vec<ClassifiedSource>, JavaScriptIndexError> {
        Ok(discover_javascript_sources(root)?)
    }
}
...
impl LanguageParser for JavaScriptParser {
    fn parse(
        ...
        source: Arc<str>,
    ) -> Result<chakra_language_index::ParsedFile, JavaScriptIndexError> {
        JavaScriptParser::parse(self, path, source)
            .map_err(|error| JavaScriptIndexError::Parse(error.to_string()))
    }
}
...
pub fn index_repository(root: &Path) -> Result<IndexReport, JavaScriptIndexError> {
    chakra_language_index::index_repository::<JavaScriptHooks>(root)
}
...
pub fn scan_repository_sources(
    ...
    chakra_language_index::scan_repository_sources::<JavaScriptHooks>(repository_root)
}
...
mod tests {
    ...
    fn cancelling_parallel_parse_joins_every_scoped_worker() -> Result<(), Box<dyn Error>> {
        ...
        let indexing = std::thread::spawn(move || {
            ...
            parse_sources_scheduled_observed::<JavaScriptHooks>(
                sources,
                WORKERS,
                1,
                &worker_cancellation,
