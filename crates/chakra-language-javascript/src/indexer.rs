//! Deterministic JavaScript syntax indexing with reusable per-file facts.
...
use crate::discovery::{DiscoveryError, discover_javascript_sources, resolve_repository_root};
use crate::parser::{JavaScriptParser, ParsedFile, SymbolDraft};
...
pub struct ReconcileReport {
    ...
    pub next_index: Option<JavaScriptSyntaxIndex>,
...
pub struct IndexReport {
    ...
    pub syntax_index: JavaScriptSyntaxIndex,
}

/// Latest JavaScript source text plus role/package metadata from the same scan.
#[derive(Debug, Clone, Default, PartialEq, Eq)]
pub struct JavaScriptSources {
    pub files: BTreeMap<RepoRelativePath, Arc<str>>,
    pub metadata: BTreeMap<RepoRelativePath, SourceMetadata>,
}

impl JavaScriptSources {
...
/// Failure to discover, read, parse, or validate the JavaScript syntax index.
#[derive(Debug, Error)]
pub enum JavaScriptIndexError {
    #[error(transparent)]
    Discovery(#[from] DiscoveryError),
    #[error("failed to read JavaScript source {path}: {source}")]
    Read {
        path: RepoRelativePath,
        #[source]
        source: io::Error,
    },
    #[error("JavaScript source {path} exceeds the {limit}-byte indexing budget")]
    SourceTooLarge {
        path: RepoRelativePath,
        limit: usize,
    },
    #[error("indexed JavaScript sources exceed the {limit}-byte repository budget")]
    RepositoryTooLarge { limit: usize },
    #[error("failed to parse JavaScript source: {0}")]
    Parse(String),
    #[error("failed to start a bounded JavaScript parser worker: {0}")]
    WorkerSpawn(#[source] io::Error),
    #[error("a bounded JavaScript parser worker panicked")]
    WorkerPanicked,
    #[error("JavaScript syntax index update failed: {0}")]
    Update(String),
    #[error(transparent)]
    Graph(#[from] GraphError),
    #[error("constructed syntax graph is inconsistent: {0}")]
    Consistency(#[from] ConsistencyError),
    #[error("JavaScript syntax indexing was cancelled")]
    Cancelled,
}
...
#[derive(Debug, Clone)]
pub struct JavaScriptSyntaxIndex {
...
impl Default for JavaScriptSyntaxIndex {
    fn default() -> Self {
        Self {
            files: BTreeMap::new(),
...
impl JavaScriptSyntaxIndex {
    pub fn from_sources(
        sources: BTreeMap<RepoRelativePath, Arc<str>>,
    ) -> Result<(Self, SymbolGraph), JavaScriptIndexError> {
    ...
    ) -> Result<(Self, SymbolGraph, LanguageBuildMetrics), JavaScriptIndexError> {
    ...
    pub fn reconcile_sources(
        &self,
        sources: BTreeMap<RepoRelativePath, Arc<str>>,
    ) -> Result<ReconcileReport, JavaScriptIndexError> {
    ...
    pub fn reconcile_classified_sources_bounded(
        ...
        let mut parser = JavaScriptParser::new().map_err(parse_error)?;
    ...
    fn materialize_graph_bounded(
        &self,
        cancellation: &IndexCancellation,
    ) -> Result<(SymbolGraph, GraphBuildReport), JavaScriptIndexError> {
    ...
    fn materialize_graph_delta(
        ...
        cancellation: &IndexCancellation,
    ) -> Result<Option<(SymbolGraph, GraphBuildReport)>, JavaScriptIndexError> {
        ...
        for path in changed_paths {
            ...
            if stable_symbol_paths.contains(path) {
                let file = self.files.get(path).ok_or_else(|| {
                    JavaScriptIndexError::Update(format!("missing changed file {path}"))
                })?;
                let ids: Vec<_> = graph
...
fn parse_sources_scheduled(
    ...
    parallel_file_threshold: usize,
    cancellation: &IndexCancellation,
) -> Result<(BTreeMap<RepoRelativePath, Arc<ParsedFile>>, ParseSchedule), JavaScriptIndexError> {
...
fn parse_sources_scheduled_inner(
    ...
    let outputs = thread::scope(
        |scope| -> Result<Vec<ParseWorkerOutput>, JavaScriptIndexError> {
            let mut workers = Vec::with_capacity(worker_count);
            let mut spawn_error = None;
            for worker in 0..worker_count {
                let name = format!("chakra-javascript-parser-{worker}");
                match thread::Builder::new().name(name).spawn_scoped(scope, || {
                    ...
                    let output = (|| {
                        let mut parser = match JavaScriptParser::new() {
                            Ok(parser) => parser,
                            Err(error) => {
            ...
            if worker_panicked {
                return Err(JavaScriptIndexError::WorkerPanicked);
            }
            if let Some(error) = spawn_error {
                return Err(JavaScriptIndexError::WorkerSpawn(error));
            }
            Ok(outputs)
        },
    )?;
    ...
    {
        return Err(JavaScriptIndexError::Parse(error));
    }
    ...
    for output in outputs {
        for (index, parsed) in output.parsed {
            let Some(slot) = ordered.get_mut(index) else {
                return Err(JavaScriptIndexError::Update(
    ...
    for ((path, _), parsed) in inputs.into_iter().zip(ordered) {
        let parsed = parsed.ok_or_else(|| {
            JavaScriptIndexError::Update("parser worker omitted an admitted source".to_owned())
        })?;
        let parsed = parsed.map_err(JavaScriptIndexError::Parse)?;
        files.insert(path, Arc::new(parsed));
    }
    Ok((
        files,
...
fn check_cancelled(cancellation: &IndexCancellation) -> Result<(), JavaScriptIndexError> {
    if cancellation.is_cancelled() {
        Err(JavaScriptIndexError::Cancelled)
    } else {
        Ok(())
    }
}
...
fn build_all_relationships(
    ...
    limit: u64,
    cancellation: &IndexCancellation,
) -> Result<BTreeMap<RepoRelativePath, Arc<RelationshipContribution>>, JavaScriptIndexError> {
...
fn parse_error(error: impl std::fmt::Display) -> JavaScriptIndexError {
    JavaScriptIndexError::Parse(error.to_string())
}
...
fn read_sources(repository_root: &Path) -> Result<JavaScriptSources, JavaScriptIndexError> {
    let files = discover_javascript_sources(repository_root)?;
    ...
    Ok(JavaScriptSources {
        files: sources,
        metadata,
    })
}

/// Builds a complete JavaScript syntax index from the actual materialized Git
/// worktree. The caller owns atomic publication into `WorkspaceEngine`.
pub fn index_repository(root: &Path) -> Result<IndexReport, JavaScriptIndexError> {
    let started = Instant::now();
    let repository_root = resolve_repository_root(root)?;
    let span = info_span!("javascript_repository_index", root = %repository_root.display());
    let _entered = span.enter();
    let sources = read_sources(&repository_root)?;
    let discovered_files = sources.files.len() as u64;
    let (syntax_index, graph) = JavaScriptSyntaxIndex::from_classified_sources(sources)?;
    ...
    info!(
        ...
        "JavaScript syntax index completed"
    );
    Ok(IndexReport {
        repository_root,
        graph,
...
/// Reads the latest Git-aware JavaScript file inventory and exact contents.
pub fn scan_repository_sources(
    repository_root: &Path,
) -> Result<JavaScriptSources, JavaScriptIndexError> {
...
mod tests {
    ...
    fn bounded_parallel_parsing_is_deterministic() -> Result<(), Box<dyn Error>> {
        ...
        for index in 0..64 {
            sources.insert(
                RepoRelativePath::new(format!("src/generated_{index:03}.js"))?,
                Arc::<str>::from(format!(
                    "export function caller_{index}() {{ helper_{index}(); }}\nexport function helper_{index}() {{}}\n"
                )),
            );
        }
        let cancellation = IndexCancellation::default();
        let (_, sequential, sequential_metrics) = JavaScriptSyntaxIndex::from_sources_scheduled(
            sources.clone(),
            GraphBuildLimits::UNLIMITED,
            1,
            1,
        ...
        let (_, parallel, parallel_metrics) = JavaScriptSyntaxIndex::from_sources_scheduled(
            sources,
            GraphBuildLimits::UNLIMITED,
            4,
            1,
    ...
    fn cancelling_parallel_parse_joins_every_scoped_worker() -> Result<(), Box<dyn Error>> {
        ...
        for index in 0..64 {
            sources.insert(
                RepoRelativePath::new(format!("src/cancel_{index:03}.js"))?,
                Arc::<str>::from(format!("export function item_{index}() {{}}\n")),
            );
        }
        let cancellation = IndexCancellation::default();
        ...
        assert!(matches!(result, Err(JavaScriptIndexError::Cancelled)));
        Ok(())
    }
    ...
    fn duplicate_call_fanout_is_stored_linearly() -> Result<(), Box<dyn Error>> {
        ...
        for index in 0..TARGETS {
            source.push_str(&format!(
                "export function target_{index}() {{ function target() {{}} }}\n"
            ));
        }
        for index in 0..CALLS {
            source.push_str(&format!(
                "export function caller_{index}() {{ target(); }}\n"
            ));
        }
        let mut sources = BTreeMap::new();
        sources.insert(
            RepoRelativePath::new("src/lib.js")?,
            Arc::<str>::from(source),
        );

        let started = Instant::now();
        let (_, graph) = JavaScriptSyntaxIndex::from_sources(sources)?;
    ...
    fn truncated_catalog_never_turns_ambiguity_into_a_unique_call() -> Result<(), Box<dyn Error>> {
        let mut sources = BTreeMap::new();
        sources.insert(
            RepoRelativePath::new("src/a_caller.js")?,
            Arc::<str>::from("export function invoke() { target(); }\n"),
        );
        sources.insert(
            RepoRelativePath::new("src/b_target.js")?,
            Arc::<str>::from("export function target() {}\n"),
        );
        sources.insert(
            RepoRelativePath::new("src/c_target.js")?,
            Arc::<str>::from("export function target() {}\n"),
        );
        let cancellation = IndexCancellation::default();
        let (_, graph, metrics) = JavaScriptSyntaxIndex::from_sources_bounded(
            sources,
            GraphBuildLimits {
                max_symbols: 2,
                max_edges: 10,
    ...
    fn metadata_change_republishes_without_reparsing_source() -> Result<(), Box<dyn Error>> {
        let path = RepoRelativePath::new("src/lib.js")?;
        let source = Arc::<str>::from("export function stable() {}\n");
        let initial = JavaScriptSources {
            files: BTreeMap::from([(path.clone(), source.clone())]),
            metadata: BTreeMap::from([(path.clone(), SourceMetadata::path_fallback(&path))]),
        };
        let (index, _) = JavaScriptSyntaxIndex::from_classified_sources(initial)?;
        let changed = JavaScriptSources {
    ...
    fn rejects_a_source_larger_than_the_file_budget() -> Result<(), Box<dyn Error>> {
        ...
        let file = File::create(repository.path().join("src/large.js"))?;
        ...
        assert!(matches!(
            error,
            JavaScriptIndexError::SourceTooLarge { limit, .. } if limit == MAX_SOURCE_FILE_BYTES
        ));
        Ok(())
    }
}
