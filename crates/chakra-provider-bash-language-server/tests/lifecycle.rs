//! Hermetic bash-language-server lifecycle regressions using a scripted stdio LSP peer.
    ...
    loop {
        ...
        if body.contains("\"method\":\"initialize\"") {
            ...
            let references = format!(
                "[{{\"uri\":\"{last_uri}\",\"range\":{{\"start\":{{\"line\":4,\"character\":2}},\"end\":{{\"line\":4,\"character\":8}}}}}}]"
            );
            if let Some(id) = request_id(&body) {
                send(id, &references)?;
            }
            ...
            let symbols = format!(
                "[{{\"name\":\"caller\",\"kind\":12,\"location\":{{\"uri\":\"{last_uri}\",\"range\":{{\"start\":{{\"line\":3,\"character\":0}},\"end\":{{\"line\":5,\"character\":1}}}}}},\"containerName\":null}}]"
            );
            if let Some(id) = request_id(&body) {
                send(id, &symbols)?;
            }
...
fn compile_fake_server(root: &Path, name: &str) -> Result<PathBuf, Box<dyn Error>> {
    ...
    let status = Command::new(rustc)
        ...
        .status()?;
    if !status.success() {
        return Err("failed to compile the fake bash_language_server peer".into());
    }
    Ok(executable)
}
...
fn materialize_fake_server(root: &Path, name: &str) -> Result<PathBuf, Box<dyn Error>> {
    ...
    let shared = match SERVER.get_or_init(|| {
        ...
        let executable = compile_fake_server(scratch.path(), "fake-bash_language_server-shared")
...
const TARGET_SOURCE: &str = "target() {\n  true\n}\ncaller() {\n  target\n}\n";
...
fn request(root: &Path, revision: Revision) -> Result<PreciseQueryRequest, Box<dyn Error>> {
    let path = RepoRelativePath::new("src/main.sh")?;
...
fn config(executable: &Path) -> BashLanguageServerConfig {
    BashLanguageServerConfig {
        command: BashLanguageServerCommand::start(executable.as_os_str().to_owned()),
...
#[test]
fn precise_incoming_call_carries_bash_language_server_provenance() -> Result<(), Box<dyn Error>> {
    let repository = tempfile::tempdir()?;
    let executable =
        materialize_fake_server(repository.path(), "fake-bash_language_server-precise")?;
    let request = request(repository.path(), Revision(1))?;
    let provider =
        BashLanguageServerProvider::start(request.workspace.clone(), config(&executable))?;
    ...
    assert_eq!(relation.provenance, Provenance::BashLanguageServer);
...
#[test]
fn shell_document_syncs_with_the_shellscript_language_id() -> Result<(), Box<dyn Error>> {
    let repository = tempfile::tempdir()?;
    let executable = materialize_fake_server(repository.path(), "fake-bash-language-server-shell")?;
    let path = RepoRelativePath::new("src/main.bash")?;
    ...
    assert_eq!(
        result.incoming[0].provenance,
        Provenance::BashLanguageServer
    );
    let opened = fs::read_to_string(executable.with_extension("lastopen"))?;
    assert!(
        opened.contains("\"languageId\":\"shellscript\""),
        "didOpen must carry the shellscript language id: {opened}"
    );
    provider.shutdown()?;
    Ok(())
}
...
fn revision_delta_syncs_only_opened_documents_with_full_text() -> Result<(), Box<dyn Error>> {
    let repository = tempfile::tempdir()?;
    let executable = materialize_fake_server(repository.path(), "fake-bash_language_server-delta")?;
    let mut request = request(repository.path(), Revision(1))?;
    let other_path = RepoRelativePath::new("src/other.sh")?;
    fs::write(
    ...
    request.workspace = workspace(
        ...
        Revision(1),
        vec![
            document(&RepoRelativePath::new("src/main.sh")?, TARGET_SOURCE),
            document(&other_path, "other() { true; }\n"),
        ],
    )?;
    let provider =
        BashLanguageServerProvider::start(request.workspace.clone(), config(&executable))?;
    ...
    let revision_two = PreciseQueryRequest {
        workspace: workspace(
            ...
            vec![
                document(&RepoRelativePath::new("src/main.sh")?, TARGET_SOURCE),
                document(&other_path, "other() { false; }\n"),
            ],
        )?,
        ..request.clone()
    };
...
fn timed_out_request_is_cancelled_and_reports_catching_up() -> Result<(), Box<dyn Error>> {
    let repository = tempfile::tempdir()?;
    let executable = materialize_fake_server(repository.path(), "fake-bash_language_server-hang")?;
    let request = request(repository.path(), Revision(1))?;
    let provider =
        BashLanguageServerProvider::start(request.workspace.clone(), config(&executable))?;
...
fn per_query_wait_budget_returns_before_the_request_timeout() -> Result<(), Box<dyn Error>> {
    let repository = tempfile::tempdir()?;
    let executable =
        materialize_fake_server(repository.path(), "fake-bash_language_server-hang-budget")?;
    let request = request(repository.path(), Revision(1))?;
    ...
    let provider = BashLanguageServerProvider::start(request.workspace.clone(), bounded)?;
...
fn transport_crash_restarts_once_then_degrades() -> Result<(), Box<dyn Error>> {
    let repository = tempfile::tempdir()?;
    let executable = materialize_fake_server(repository.path(), "fake-bash_language_server-crash")?;
    let request = request(repository.path(), Revision(1))?;
    let provider =
        BashLanguageServerProvider::start(request.workspace.clone(), config(&executable))?;
...
fn missing_navigation_capabilities_degrades() -> Result<(), Box<dyn Error>> {
    let repository = tempfile::tempdir()?;
    let executable = materialize_fake_server(
        repository.path(),
        "fake-bash-language-server-no-capabilities",
    )?;
    let request = request(repository.path(), Revision(1))?;
    let provider =
        BashLanguageServerProvider::start(request.workspace.clone(), config(&executable))?;
...
fn shutdown_reaps_provider_process_group_descendants() -> Result<(), Box<dyn Error>> {
    let repository = tempfile::tempdir()?;
    let executable =
        materialize_fake_server(repository.path(), "fake-bash_language_server-spawn-child")?;
    let request = request(repository.path(), Revision(1))?;
    let provider =
        BashLanguageServerProvider::start(request.workspace.clone(), config(&executable))?;
    let result = provider.enrich(request);
