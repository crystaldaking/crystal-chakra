...
pub(crate) fn convert_references(
    ...
    for reference in references {
        ...
        relations.push(PreciseRelation {
            ...
            provenance: Provenance::BashLanguageServer,
        });
    }
    relations
}
...
mod tests {
    ...
    #[test]
    fn references_map_to_enclosing_shell_functions() -> Result<(), Box<dyn Error>> {
        let repository = TempDir::new()?;
        let path = RepoRelativePath::new("src/main.sh")?;
        let source: Arc<str> = Arc::from("caller() {\n  target\n  target\n}\n");
        ...
        let references = vec![
            ...
            Location::new(uri, Range::new(Position::new(2, 2), Position::new(2, 8))),
        ];
        ...
        assert_eq!(relations[0].provenance, Provenance::BashLanguageServer);
        assert!(!truncated);
        Ok(())
    }
