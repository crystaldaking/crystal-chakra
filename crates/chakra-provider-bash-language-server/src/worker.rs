...
//!
//! bash-language-server has no rust-analyzer-style `experimental/serverStatus`
...
use crate::{BashLanguageServerConfig, Command, SharedState};
...
pub(crate) enum ProviderError {
    #[error("bash-language-server transport failed: {0}")]
    Transport(String),
    #[error("timed out waiting for the bash-language-server response")]
    Timeout,
    #[error("the bash-language-server request was cancelled by its caller")]
    Cancelled,
    #[error("bash-language-server request failed ({code}): {message}")]
    Server { code: i32, message: String },
    #[error("invalid bash-language-server response: {0}")]
    InvalidResponse(#[from] serde_json::Error),
    #[error("bash-language-server does not advertise definition, references, and document symbols")]
    Unsupported,
...
impl Worker {
    pub(crate) fn new(
        ...
        let (workspace_documents, workspace_source_bytes) =
            initial_workspace.document_stats_matching(bash_language_server_language);
    ...
    fn query_with_owned_session(
        ...
        let mut session = self.session.take().ok_or_else(|| {
            ProviderError::Transport("no active bash-language-server process".to_owned())
        })?;
    ...
    fn query_session(
        ...
        // proves bash-language-server consumed the current document generation.
        ...
        Ok(PreciseQueryResult {
            ...
            fallback_cause: None,
            incoming,
            // bash-language-server has no callHierarchy. Chakra's bounded
            // Tree-sitter function-call graph remains the outgoing equivalent.
            outgoing: Vec::new(),
            incoming_truncated,
    ...
    fn synchronize_documents(
        ...
        let target_document = workspace
            ...
            .filter(|document| bash_language_server_language(document.language))
            .ok_or(ProviderError::InvalidPosition)?;
    ...
    fn start_session(&mut self) -> Result<(), ProviderError> {
        self.set_progress(ProviderProgress {
            ...
            source: ProviderProgressSource::Chakra,
            message: Some("starting the bash-language-server process".to_owned()),
            percentage: None,
        });
    ...
    fn drain_session_events(&mut self) {
        ...
        if closed {
            ...
            self.set_state(
                ProviderState::Degraded,
                None,
                Some("bash-language-server process closed its output".to_owned()),
            );
        } else {
            self.session = Some(session);
        }
...
fn language_id(_path: &RepoRelativePath) -> &'static str {
    "shellscript"
}
...
mod tests {
    ...
    fn language_ids_follow_the_file_extension() -> Result<(), Box<dyn std::error::Error>> {
        assert_eq!(
            language_id(&RepoRelativePath::new("src/a.sh")?),
            "shellscript"
        );
        Ok(())
    }
