//! Hermetic clangd lifecycle regressions using a tiny scripted stdio LSP peer.
...
use chakra_provider_clangd::{ClangdCommand, ClangdConfig, ClangdProvider};
...
fn main() -> io::Result<()> {
    ...
    loop {
        ...
        if body.contains("\"method\":\"initialize\"") {
            ...
            let item = format!(
                "[{{\"name\":\"target\",\"kind\":12,\"uri\":\"{last_uri}\",\"range\":{{\"start\":{{\"line\":2,\"character\":4}},\"end\":{{\"line\":2,\"character\":20}}}},\"selectionRange\":{{\"start\":{{\"line\":2,\"character\":9}},\"end\":{{\"line\":2,\"character\":15}}}}}}]"
            );
            if let Some(id) = request_id(&body) {
                send(id, &item)?;
            }
            ...
            let call = format!(
                "[{{\"from\":{{\"name\":\"caller\",\"kind\":12,\"uri\":\"{last_uri}\",\"range\":{{\"start\":{{\"line\":3,\"character\":4}},\"end\":{{\"line\":3,\"character\":31}}}},\"selectionRange\":{{\"start\":{{\"line\":3,\"character\":9}},\"end\":{{\"line\":3,\"character\":15}}}}}},\"fromRanges\":[{{\"start\":{{\"line\":3,\"character\":20}},\"end\":{{\"line\":3,\"character\":26}}}}]}}]"
            );
            if let Some(id) = request_id(&body) {
                send(id, &call)?;
            }
...
fn compile_fake_server(root: &Path, name: &str) -> Result<PathBuf, Box<dyn Error>> {
    ...
    let status = Command::new(rustc)
        ...
        .status()?;
    if !status.success() {
        return Err("failed to compile the fake clangd peer".into());
    }
    Ok(executable)
}
...
fn materialize_fake_server(root: &Path, name: &str) -> Result<PathBuf, Box<dyn Error>> {
    ...
    let shared = match SERVER.get_or_init(|| {
        ...
        let executable = compile_fake_server(scratch.path(), "fake-clangd-shared")
...
fn config(executable: &Path) -> ClangdConfig {
    ClangdConfig {
        command: ClangdCommand::stdio(executable.as_os_str().to_owned()),
...
#[test]
fn precise_incoming_call_carries_clangd_provenance() -> Result<(), Box<dyn Error>> {
    let repository = tempfile::tempdir()?;
    let executable = materialize_fake_server(repository.path(), "fake-clangd-precise")?;
    let request = request(repository.path(), Revision(1))?;
    let provider = ClangdProvider::start(request.workspace.clone(), config(&executable))?;
    ...
    assert_eq!(relation.provenance, Provenance::Clangd);
...
fn revision_delta_syncs_only_opened_documents_with_full_text() -> Result<(), Box<dyn Error>> {
    let repository = tempfile::tempdir()?;
    let executable = materialize_fake_server(repository.path(), "fake-clangd-delta")?;
    let mut request = request(repository.path(), Revision(1))?;
    let other_path = RepoRelativePath::new("src/other.cpp")?;
    fs::write(
        repository.path().join(other_path.as_str()),
        "namespace other { int value = 1; }\n",
    )?;
    request.workspace = workspace(
        repository.path(),
        Revision(1),
        vec![
            document(&RepoRelativePath::new("src/index.cpp")?, TARGET_SOURCE),
            document(&other_path, "namespace other { int value = 1; }\n"),
        ],
    )?;
    let provider = ClangdProvider::start(request.workspace.clone(), config(&executable))?;
    ...
    let revision_two = PreciseQueryRequest {
        workspace: workspace(
            ...
            vec![
                document(&RepoRelativePath::new("src/index.cpp")?, TARGET_SOURCE),
                document(&other_path, "namespace other { int value = 2; }\n"),
            ],
        )?,
        ..request.clone()
    };
...
fn timed_out_request_is_cancelled_and_reports_catching_up() -> Result<(), Box<dyn Error>> {
    let repository = tempfile::tempdir()?;
    let executable = materialize_fake_server(repository.path(), "fake-clangd-hang")?;
    let request = request(repository.path(), Revision(1))?;
    let provider = ClangdProvider::start(request.workspace.clone(), config(&executable))?;
...
fn per_query_wait_budget_returns_before_the_request_timeout() -> Result<(), Box<dyn Error>> {
    let repository = tempfile::tempdir()?;
    let executable = materialize_fake_server(repository.path(), "fake-clangd-hang-budget")?;
    let request = request(repository.path(), Revision(1))?;
    ...
    let provider = ClangdProvider::start(request.workspace.clone(), bounded)?;
...
fn transport_crash_restarts_once_then_degrades() -> Result<(), Box<dyn Error>> {
    let repository = tempfile::tempdir()?;
    let executable = materialize_fake_server(repository.path(), "fake-clangd-crash")?;
    let request = request(repository.path(), Revision(1))?;
    let provider = ClangdProvider::start(request.workspace.clone(), config(&executable))?;
...
fn missing_call_hierarchy_capability_degrades() -> Result<(), Box<dyn Error>> {
    let repository = tempfile::tempdir()?;
    let executable = materialize_fake_server(repository.path(), "fake-clangd-no-hierarchy")?;
    let request = request(repository.path(), Revision(1))?;
    let provider = ClangdProvider::start(request.workspace.clone(), config(&executable))?;
...
fn shutdown_reaps_provider_process_group_descendants() -> Result<(), Box<dyn Error>> {
    let repository = tempfile::tempdir()?;
    let executable = materialize_fake_server(repository.path(), "fake-clangd-spawn-child")?;
    let request = request(repository.path(), Revision(1))?;
    let provider = ClangdProvider::start(request.workspace.clone(), config(&executable))?;
    let result = provider.enrich(request);
    assert_eq!(result.state, ProviderState::Ready);
