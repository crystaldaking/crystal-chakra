...
//!
//! clangd has no rust-analyzer-style `experimental/serverStatus` quiescence
//! signal. Readiness is instead proven by a post-synchronization request
//! barrier (ADR-0039): after the current document generation is sent, the
...
use crate::{ClangdConfig, Command, SharedState};
...
pub(crate) enum ProviderError {
    #[error("clangd transport failed: {0}")]
    Transport(String),
    #[error("timed out waiting for the clangd response")]
    Timeout,
    #[error("the clangd request was cancelled by its caller")]
    Cancelled,
    #[error("clangd request failed ({code}): {message}")]
    Server { code: i32, message: String },
    #[error("invalid clangd response: {0}")]
    InvalidResponse(#[from] serde_json::Error),
    #[error("clangd does not advertise call hierarchy")]
    Unsupported,
...
impl Worker {
    pub(crate) fn new(
        ...
        let (workspace_documents, workspace_source_bytes) =
            initial_workspace.document_stats(Language::Cpp);
    ...
    fn query_with_owned_session(
        ...
        let mut session = self
            .session
            .take()
            .ok_or_else(|| ProviderError::Transport("no active clangd process".to_owned()))?;
    ...
    fn synchronize_documents(
        ...
        let target_document = workspace
            ...
            .filter(|document| document.language == Language::Cpp)
            .ok_or(ProviderError::InvalidPosition)?;
    ...
    fn start_session(&mut self) -> Result<(), ProviderError> {
        self.set_progress(ProviderProgress {
            ...
            source: ProviderProgressSource::Chakra,
            message: Some("starting the clangd process".to_owned()),
            percentage: None,
        });
    ...
    fn drain_session_events(&mut self) {
        ...
        if closed {
            ...
            self.set_state(
                ProviderState::Degraded,
                None,
                Some("clangd process closed its output".to_owned()),
            );
        } else {
            self.session = Some(session);
        }
...
fn language_id(path: &RepoRelativePath) -> &'static str {
    match std::path::Path::new(path.as_str())
        .extension()
...
mod tests {
    ...
    fn language_ids_follow_the_file_extension() -> Result<(), Box<dyn std::error::Error>> {
        assert_eq!(language_id(&RepoRelativePath::new("src/a.cpp")?), "cpp");
        assert_eq!(language_id(&RepoRelativePath::new("src/a.hpp")?), "cpp");
        assert_eq!(language_id(&RepoRelativePath::new("src/a.c")?), "c");
        Ok(())
    }
}
