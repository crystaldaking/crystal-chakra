...
//! syntax snapshot, and every converted fact carries `Provenance::Clangd`.
...
mod tests {
    ...
    fn cpp_document(path: &RepoRelativePath, source: &str) -> ProviderDocument {
        ProviderDocument {
            path: path.clone(),
            source: Arc::from(source),
            language: chakra_domain::symbol::Language::Cpp,
        }
    }
    ...
    #[test]
    fn incoming_relations_carry_clangd_provenance_and_bounded_call_sites()
    -> Result<(), Box<dyn Error>> {
        ...
        assert_eq!(converted[0].provenance, Provenance::Clangd);
        assert!(!truncated);
        Ok(())
    }
