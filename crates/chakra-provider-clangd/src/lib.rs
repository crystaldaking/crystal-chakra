...
//! (issue #94); this crate keeps only clangd-specific seams: command
//! discovery, defaults, the c/cpp language ids, and the call-hierarchy
//! capability gate.
//!
...
impl From<ClangdCommand> for ProviderCommandSpec {
    fn from(command: ClangdCommand) -> Self {
        Self {
...
impl From<ClangdConfig> for WorkerConfig {
    fn from(config: ClangdConfig) -> Self {
        Self {
...
/// clangd language hooks: C/C++ documents synchronize through the session
/// and the precise surface is the LSP call-hierarchy trio verified at
/// initialization.
#[derive(Debug, Clone, Copy, Default)]
struct ClangdHooks;

impl ProviderHooks for ClangdHooks {
    ...
    fn query(
        ...
        CallHierarchyDriver.query(channel, request, deadlines, Provenance::Clangd)
    }
}

/// Owned clangd process and worker lifecycle.
pub struct ClangdProvider {
    inner: Arc<ProviderHandle<ClangdHooks>>,
}

impl std::fmt::Debug for ClangdProvider {
...
impl ClangdProvider {
    ...
    pub fn start(
        ...
        let inner = ProviderHandle::start(initial_workspace, config.into(), ClangdHooks)?;
        Ok(Arc::new(Self { inner }))
    }

    /// Idempotent cooperative shutdown followed by joining the owned worker.
    /// The owned process group is terminated, so no clangd child remains.
    pub fn shutdown(&self) -> Result<(), ShutdownError> {
        self.inner.shutdown()
    }
}

impl chakra_engine::PreciseProvider for ClangdProvider {
    fn name(&self) -> &'static str {
        self.inner.name()
    }
