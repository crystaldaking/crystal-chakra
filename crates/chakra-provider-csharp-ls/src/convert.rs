...
//! syntax snapshot, and every converted fact carries `Provenance::CsharpLs`.
...
pub(crate) fn convert_incoming(
    ...
    for call in calls {
        ...
        result.push(PreciseRelation {
            ...
            occurrence_count,
            call_sites,
            provenance: Provenance::CsharpLs,
        });
    }
    result
}
...
mod tests {
    ...
    fn csharp_document(path: &RepoRelativePath, source: &str) -> ProviderDocument {
        ProviderDocument {
            path: path.clone(),
            source: Arc::from(source),
            language: chakra_domain::symbol::Language::CSharp,
        }
    }
    ...
    #[test]
    fn incoming_relations_carry_csharp_ls_provenance_and_bounded_call_sites()
    -> Result<(), Box<dyn Error>> {
        ...
        let workspace = ProviderWorkspace::from_documents(
            ...
            vec![csharp_document(
                &path,
                "static class Sample { static void target() {} }\n",
            )],
        );
        ...
        assert_eq!(converted[0].provenance, Provenance::CsharpLs);
        assert!(!truncated);
        Ok(())
    }
    ...
    fn relations_outside_the_workspace_are_dropped_before_the_limit() -> Result<(), Box<dyn Error>>
    {
        ...
        let workspace = ProviderWorkspace::from_documents(
            ...
            vec![csharp_document(
                &path,
                "static class Sample { static void inside() {} }\n",
            )],
        );
