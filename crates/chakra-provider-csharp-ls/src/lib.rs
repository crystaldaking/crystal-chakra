...
pub enum StartError {
    ...
    InvalidTimeout,
    #[error("failed to spawn the csharp-ls owner thread: {0}")]
    ThreadSpawn(#[source] std::io::Error),
}

#[derive(Debug, Error)]
pub enum ShutdownError {
    #[error("csharp-ls owner thread panicked")]
    WorkerPanicked,
    #[error("csharp-ls owner lock is poisoned")]
    LockPoisoned,
}
...
impl fmt::Debug for CsharpLsProvider {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("CsharpLsProvider")
...
impl CsharpLsProvider {
    ...
    pub fn start(
        ...
        let worker = thread::Builder::new()
            .name("chakra-csharp-ls".to_owned())
            .spawn(move || {
                Worker::new(
                    receiver,
                    worker_shared,
...
impl PreciseProvider for CsharpLsProvider {
    ...
    fn last_error(&self) -> Option<String> {
        CsharpLsProvider::last_error(self)
    }

    fn progress(&self) -> Option<ProviderProgress> {
        CsharpLsProvider::progress(self)
    }

    fn metrics(&self) -> Option<ProviderMetrics> {
        CsharpLsProvider::metrics(self)
    }
    ...
    fn shutdown(&self) -> Result<(), ProviderShutdownError> {
        CsharpLsProvider::shutdown(self)
