...
//!
//! csharp-ls has no rust-analyzer-style `experimental/serverStatus` quiescence
//! signal. Readiness is instead proven by a post-synchronization request
//! barrier (ADR-0037): after the current document generation is sent, the
...
use crate::{Command, CsharpLsConfig, SharedState};
...
pub(crate) enum ProviderError {
    #[error("csharp-ls transport failed: {0}")]
    Transport(String),
    #[error("timed out waiting for the csharp-ls response")]
    Timeout,
    #[error("the csharp-ls request was cancelled by its caller")]
    Cancelled,
    #[error("csharp-ls request failed ({code}): {message}")]
    Server { code: i32, message: String },
    #[error("invalid csharp-ls response: {0}")]
    InvalidResponse(#[from] serde_json::Error),
    #[error("csharp-ls does not advertise call hierarchy")]
    Unsupported,
...
impl Worker {
    pub(crate) fn new(
        ...
        let (workspace_documents, workspace_source_bytes) =
            initial_workspace.document_stats(Language::CSharp);
    ...
    fn query_with_owned_session(
        ...
        let mut session = self
            .session
            .take()
            .ok_or_else(|| ProviderError::Transport("no active csharp-ls process".to_owned()))?;
    ...
    fn synchronize_documents(
        ...
        let target_document = workspace
            ...
            .filter(|document| document.language == Language::CSharp)
            .ok_or(ProviderError::InvalidPosition)?;
    ...
    fn start_session(&mut self) -> Result<(), ProviderError> {
        self.set_progress(ProviderProgress {
            ...
            source: ProviderProgressSource::Chakra,
            message: Some("starting the csharp-ls process".to_owned()),
            percentage: None,
        });
    ...
    fn drain_session_events(&mut self) {
        ...
        if closed {
            ...
            self.set_state(
                ProviderState::Degraded,
                None,
                Some("csharp-ls process closed its output".to_owned()),
            );
        } else {
            self.session = Some(session);
        }
...
mod tests {
    ...
    fn language_ids_follow_the_file_extension() -> Result<(), Box<dyn std::error::Error>> {
        assert_eq!(language_id(&RepoRelativePath::new("src/a.cs")?), "csharp");
        Ok(())
    }
}
