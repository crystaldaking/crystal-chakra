...
fn cargo_metadata_scans(
    ...
    for manifest in manifests {
        operation.check()?;
        if covered.contains(&manifest) {
            continue;
        }
        if invocations == MAX_CARGO_METADATA_INVOCATIONS {
            issues.push(ProjectManifestIssue {
                manifest,
                kind: ProjectManifestIssueKind::ProbeOmitted,
            });
            continue;
        }
...
/// Unreadable, oversized, bounded-out, or unparsable manifests are recorded
/// as distinct issues.
fn composer_package_facts(
    ...
    for (index, manifest) in manifests_named(metadata_inputs, "composer.json")
        .into_iter()
        .enumerate()
    {
        operation.check()?;
        if index >= MAX_COMPOSER_MANIFESTS {
            issues.push(ProjectManifestIssue {
                manifest,
                kind: ProjectManifestIssueKind::ProbeOmitted,
            });
        ...
        if bytes.len() > MAX_COMPOSER_MANIFEST_BYTES {
            issues.push(ProjectManifestIssue {
                manifest: manifest.clone(),
                kind: ProjectManifestIssueKind::ProbeOmitted,
            });
