...
#[test]
fn composer_manifest_probe_limit_is_reported_honestly() -> Result<(), Box<dyn Error>> {
    let repository = repository()?;
    let root = repository.path();
    for index in 0..=MAX_COMPOSER_MANIFESTS {
        write(
            root,
            &format!("packages/{index:03}/composer.json"),
            &format!(r#"{{ "name": "acme/package-{index:03}" }}"#),
        )?;
    }
    command(root, "git", &["add", "."])?;
    ...
    assert_eq!(
        model
            ...
            .filter(|unit| unit.kind == ProjectUnitKind::ComposerPackage)
            .count(),
        MAX_COMPOSER_MANIFESTS
    );
    assert_eq!(
        model
            .issues
            .iter()
            .filter(|issue| issue.kind == ProjectManifestIssueKind::ProbeOmitted)
            .count(),
        1,
        "the bounded-out manifest must remain observable"
    );
    Ok(())
}
