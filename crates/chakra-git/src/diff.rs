//! Fixed-argument Git implementation of the workspace diff provider.
//!
//! The provider asks Git to compare a resolved commit baseline with the final
...
fn is_supported_source(path: &str) -> bool {
    crate::discovery::source_language(path).is_some()
}

fn raw_is_supported_source(raw: &[u8]) -> Result<bool, WorkspaceDiffError> {
    if !crate::discovery::raw_may_be_source(raw) {
        return Ok(false);
    }
