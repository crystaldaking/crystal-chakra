//! Deterministic Go syntax indexing with reusable per-file facts.
...
use crate::discovery::{DiscoveryError, discover_go_sources, resolve_repository_root};
use crate::parser::{GoParser, ParsedFile, SymbolDraft};
...
pub struct ReconcileReport {
    ...
    pub next_index: Option<GoSyntaxIndex>,
...
pub struct IndexReport {
    ...
    pub syntax_index: GoSyntaxIndex,
}
...
pub enum GoIndexError {
    ...
    #[error("failed to read Go source {path}: {source}")]
    Read {
        path: RepoRelativePath,
        #[source]
        source: io::Error,
    },
    #[error("Go source {path} exceeds the {limit}-byte indexing budget")]
    SourceTooLarge {
        path: RepoRelativePath,
        limit: usize,
    },
    #[error("indexed Go sources exceed the {limit}-byte repository budget")]
    RepositoryTooLarge { limit: usize },
    #[error("failed to parse Go source: {0}")]
    Parse(String),
    #[error("failed to start a bounded Go parser worker: {0}")]
    WorkerSpawn(#[source] io::Error),
    #[error("a bounded Go parser worker panicked")]
    WorkerPanicked,
    #[error("Go syntax index update failed: {0}")]
    Update(String),
    #[error(transparent)]
    Graph(#[from] GraphError),
    #[error("constructed syntax graph is inconsistent: {0}")]
    Consistency(#[from] ConsistencyError),
    #[error("Go syntax indexing was cancelled")]
    Cancelled,
}
...
impl Default for GoSyntaxIndex {
    fn default() -> Self {
        Self {
            files: BTreeMap::new(),
...
impl GoSyntaxIndex {
    ...
    ) -> Result<(Self, SymbolGraph), GoIndexError> {
    ...
    ) -> Result<(Self, SymbolGraph, LanguageBuildMetrics), GoIndexError> {
    ...
    pub fn reconcile_sources(
        &self,
        sources: BTreeMap<RepoRelativePath, Arc<str>>,
    ) -> Result<ReconcileReport, GoIndexError> {
    ...
    pub fn reconcile_classified_sources_bounded(
        ...
        let mut parser = GoParser::new().map_err(parse_error)?;
    ...
    fn materialize_graph_bounded(
        &self,
        cancellation: &IndexCancellation,
    ) -> Result<(SymbolGraph, GraphBuildReport), GoIndexError> {
    ...
    fn materialize_graph_delta(
        ...
        cancellation: &IndexCancellation,
    ) -> Result<Option<(SymbolGraph, GraphBuildReport)>, GoIndexError> {
        ...
        for path in changed_paths {
            ...
            if stable_symbol_paths.contains(path) {
                let file = self
                    .files
                    .get(path)
                    .ok_or_else(|| GoIndexError::Update(format!("missing changed file {path}")))?;
                let ids: Vec<_> = graph
...
fn parse_sources_scheduled(
    ...
    parallel_file_threshold: usize,
    cancellation: &IndexCancellation,
) -> Result<(BTreeMap<RepoRelativePath, Arc<ParsedFile>>, ParseSchedule), GoIndexError> {
...
fn parse_sources_scheduled_inner(
    ...
    let outputs = thread::scope(|scope| -> Result<Vec<ParseWorkerOutput>, GoIndexError> {
        let mut workers = Vec::with_capacity(worker_count);
        let mut spawn_error = None;
        for worker in 0..worker_count {
            let name = format!("chakra-go-parser-{worker}");
        ...
        if worker_panicked {
            return Err(GoIndexError::WorkerPanicked);
        }
        if let Some(error) = spawn_error {
            return Err(GoIndexError::WorkerSpawn(error));
        }
        Ok(outputs)
    })?;
    check_cancelled(cancellation)?;
    ...
    {
        return Err(GoIndexError::Parse(error));
    }
    ...
    for output in outputs {
        for (index, parsed) in output.parsed {
            let Some(slot) = ordered.get_mut(index) else {
                return Err(GoIndexError::Update(
    ...
    for ((path, _), parsed) in inputs.into_iter().zip(ordered) {
        let parsed = parsed.ok_or_else(|| {
            GoIndexError::Update("parser worker omitted an admitted source".to_owned())
        })?;
        let parsed = parsed.map_err(GoIndexError::Parse)?;
        files.insert(path, Arc::new(parsed));
    }
    Ok((
        files,
...
fn check_cancelled(cancellation: &IndexCancellation) -> Result<(), GoIndexError> {
    if cancellation.is_cancelled() {
        Err(GoIndexError::Cancelled)
    } else {
        Ok(())
    }
}
...
fn build_all_relationships(
    ...
    limit: u64,
    cancellation: &IndexCancellation,
) -> Result<BTreeMap<RepoRelativePath, Arc<RelationshipContribution>>, GoIndexError> {
...
fn parse_error(error: impl std::fmt::Display) -> GoIndexError {
    GoIndexError::Parse(error.to_string())
}
...
fn read_sources(repository_root: &Path) -> Result<GoSources, GoIndexError> {
    let files = discover_go_sources(repository_root)?;
    ...
    for discovered in files {
        ...
        file.take((MAX_SOURCE_FILE_BYTES + 1) as u64)
            ...
            .map_err(|source| GoIndexError::Read {
                path: path.clone(),
                source,
            })?;
        if source.len() > MAX_SOURCE_FILE_BYTES {
            return Err(GoIndexError::SourceTooLarge {
                path,
        ...
        total_bytes =
            total_bytes
                ...
                .ok_or(GoIndexError::RepositoryTooLarge {
                    limit: MAX_REPOSITORY_SOURCE_BYTES,
                })?;
        if total_bytes > MAX_REPOSITORY_SOURCE_BYTES {
            return Err(GoIndexError::RepositoryTooLarge {
                limit: MAX_REPOSITORY_SOURCE_BYTES,
            });
        }
...
pub fn index_repository(root: &Path) -> Result<IndexReport, GoIndexError> {
    ...
    let span = info_span!("go_repository_index", root = %repository_root.display());
    let _entered = span.enter();
    let sources = read_sources(&repository_root)?;
    let discovered_files = sources.files.len() as u64;
    let (syntax_index, graph) = GoSyntaxIndex::from_classified_sources(sources)?;
