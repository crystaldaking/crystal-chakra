//! End-to-end syntax coverage over the Go fixture.
...
use chakra_language_go::{IndexMetrics, index_repository};
...
#[test]
fn fixture_extracts_packages_types_imports_tests_and_calls() -> Result<(), Box<dyn Error>> {
    ...
    for (qualified_name, kind) in [
        ("payments::PaymentProvider", SymbolKind::Interface),
        ("payments::Envelope", SymbolKind::Struct),
        ("payments::PaymentService", SymbolKind::Struct),
        ("payments::PaymentService::Refund", SymbolKind::Method),
        ("payments::providerRefund", SymbolKind::Function),
        ("payments::serviceRefund", SymbolKind::Function),
        ("payments::handleRefund", SymbolKind::Function),
        ("payments::TestRefundFlow", SymbolKind::Test),
    ] {
        assert!(
    ...
    assert!(graph.symbols().iter().all(|symbol| {
        symbol.key.language == Language::Go
    ...
    let resolved_calls = graph
        .symbols()
        .iter()
        .flat_map(|symbol| graph.call_sites_from(symbol.id))
        .filter(|call| matches!(call.resolution, CallResolution::Resolved { .. }))
        .count();
    assert!(resolved_calls >= 3);
    Ok(())
}

#[test]
fn fixture_roles_and_packages_come_from_go_module_boundaries() -> Result<(), Box<dyn Error>> {
    ...
    assert_eq!(coverage.go_module_metadata_files, 7);

    for (path, role) in [
        ("src/payment_service.go", SourceRole::Production),
        ("tests/payment_flow_test.go", SourceRole::Test),
        ("vendor/external.go", SourceRole::Vendor),
        ("generated/payment.pb.go", SourceRole::Generated),
    ] {
        let metadata = graph
        ...
        assert_eq!(
            ...
            Some("example.com/chakra/payments")
        );
    }
    Ok(())
}
...
#[test]
fn malformed_go_keeps_valid_symbols_and_reports_diagnostics() -> Result<(), Box<dyn Error>> {
    let repository = fixture_repository()?;
    fs::write(
        repository.path().join("src/payment_controller.go"),
        "package payments\nfunc retainedMarker() int { return 1 }\ntype Broken struct {\n",
    )?;
    let report = index_repository(repository.path())?;
    ...
    assert!(diagnostics.diagnostics.iter().all(|diagnostic| {
        diagnostic.language == Language::Go
    ...
    assert!(
        report
            ...
            .symbols()
            .iter()
            .any(|symbol| { symbol.key.qualified_name == "payments::retainedMarker" })
    );
    Ok(())
}
...
fn callers_and_incremental_reconcile_use_go_function_edges() -> Result<(), Box<dyn Error>> {
    ...
    assert_eq!(
        ...
        BTreeSet::from([
            ...
            "payments::serviceRefund",
        ])
    );

    let report = index_repository(repository.path())?;
    let service = repository.path().join("src/payment_service.go");
    let source = fs::read_to_string(&service)?;
    fs::write(
        &service,
        source.replace("paymentServiceMarker", "paymentServiceMarkerChanged"),
    )?;
    let sources = chakra_language_go::scan_repository_sources(repository.path())?;
