...
Chakra has independent rust-analyzer, vtsls, and pyright adapters, but the
workspace engine could install only one provider. Starting every configured
provider eagerly would also make polyglot workspaces consume processes and
memory without a query needing precise enrichment. Issue #26 requires at
least three simultaneous providers while retaining deterministic resource
bounds, exact-revision routing, syntax fallback, cancellation, and owned
shutdown.
...
- Rust, TypeScript/JavaScript, and Python precise providers can coexist in one
  materialized polyglot workspace without cross-language routing.
